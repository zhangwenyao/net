...
int cal_lkkProb_gaussS2(double& s2, VDouble& deg2ArrVal, VNodeType& degArrVal, VNodeType& degArrSize, const NodeType xi, const double r)
{
    ...
    while(ds < 1.E9) {
        ...
        pd = &deg2ArrVal[0];
        temp = -0.5 / s2;
        for(NodeType i = 0, *ps = &degArrSize[0], *pk = &degArrVal[0]; i < degSize; i++) {
            double t = xi0 - *pd;
    ...
    while(ds > 1.E-9) {
        ds /= 2;
        if((exi <= 0.5 && sum2 >= 1.E-9) || (exi >= 0.5 && sum2 <= -1.E-9)) s2 -= ds;
        else if((exi <= 0.5 && sum2 <= -1.E-9) || (exi >= 0.5 && sum2 >= 1.E-9)) s2 += ds;
        else break;
        sum = sum2 = 0.;
        pd = &deg2ArrVal[0];
        temp = -0.5 / s2;
...
int cal_lkkProb_expS2(double& s2, VDouble& deg2ArrVal, VNodeType& degArrVal, VNodeType& degArrSize, const LinkType linkSize, const NodeType xi, const double r)
{
    ...
    while(ds < 1.E9) {
        ...
        pxj = &deg2ArrVal[0];
        t2 = 1. / (2. * linkSize);
        for(NodeType j = 0, *pj = &degArrVal[0], *psj = &degArrSize[0]; j < degSize; j++) {
            double t = (xi0 - *pxj) * s2;
            if(t > 0) t = -t;
...
int cal_lkkProb_exp0(VVDouble& lkkProb, const VDouble& s2, const VDouble& deg2ArrVal, const VNodeType& degArrVal, const double r)
{
    const NodeType degSize = degArrVal.size();
    for(NodeType i = 0; i < degSize; i++) {
        double xi = deg2ArrVal[i];
...
int degArrVal_2_deg2ArrVal(VDouble& deg2ArrVal, const VNodeType& degArrSize, const VNodeType& degArrVal, const double linkSize, const int dirFlag)
{
    const NodeType degSize = degArrVal.size();
...
{
    netWeight = 0;
    for(VWeightTypeCItr i = degArrWeight.begin(); i != degArrWeight.end(); i++) netWeight += *i;
    return 0;
}
...
int cal_nodeNeiAveDeg2(VDouble& nodeNeiAveDeg2, const VVNodeType& p2p, const VDouble& deg2ArrVal, MNodeType& degArrNo)
{
    ...
    for(NodeType i = 0; i < nodeSize; i++) {
        ...
        for(VNodeTypeCItr n = p2p[i].begin(); n != p2p[i].end(); n++) nodeNeiAveDeg2[i] += deg2ArrVal[degArrNo[p2p[*n].size()]];
...
int cal_nodeNeiAveDeg2_nodeDeg(VDouble& nodeNeiAveDeg2, const VVNodeType& p2p, const VNodeType& nodeDeg, const VDouble& deg2ArrVal, MNodeType& degArrNo)
{
    ...
    for(NodeType i = 0; i < nodeSize; i++) {
        if(p2p[i].size() <= 0) continue;
        for(VNodeTypeCItr n = p2p[i].begin(); n != p2p[i].end(); n++) nodeNeiAveDeg2[i] += deg2ArrVal[degArrNo[nodeDeg[*n]]];
        nodeNeiAveDeg2[i] /= p2p[i].size();
    }
    return 0;
}
...
{
    if(!weightFlag) return cal_nodeNeiAveDeg2_nodeDeg(nodeNeiAveDeg2, p2p, nodeDegIn, deg2ArrValIn, degArrNoIn);
    ...
    for(NodeType i = 0; i < nodeSize; i++) {
        if(p2p[i].size() <= 0) continue;
        double l = 0;
        for(NodeType j = 0; j < p2p[i].size(); j++) l += (double)vvweight[i][j] * deg2ArrValIn[degArrNoIn[nodeDegIn[p2p[i][j]]]];
...
{
    if(!weightFlag) return cal_nodeNeiAveDeg2_nodeDeg(nodeNeiAveDeg2, p2p, nodeDeg, deg2ArrVal, degArrNo);
    if(p2p.empty() || !nodeNeiAveDeg2.empty() || vvweight.size() != p2p.size()){
        ERROR();
        return -1;
    }
    ...
    for(NodeType i = 0; i < nodeSize; i++) {
        ...
        for(NodeType j = 0; j < p2p[i].size(); j++){
            nodeNeiAveDeg2[i] += (double)vvweight[i][j] * deg2ArrVal[degArrNo[nodeDeg[p2p[i][j]]]];
            nodeNeiAveDeg2[p2p[i][j]] += (double)vvweight[i][j] * deg2ArrVal[degArrNo[nodeDeg[i]]];
        }
    }
    for(NodeType i = 0; i < nodeSize; i++)
...
int cal_neiAveDeg2(VDouble& neiAveDeg2, const VDouble& nodeNeiAveDeg2, const VNodeType& nodeDeg, const VNodeType& degArrSize, MNodeType& degArrNo, const VNodeType& degArrVal)
{
    ...
    for(NodeType i = 0; i < nodeSize; i++) neiAveDeg2[degArrNo[nodeDeg[i]]] += nodeNeiAveDeg2[i];
    for(NodeType i = 0; i < degSize; i++)
...
{
    if(!weightFlag) return cal_neiAveDeg2(neiAveDeg2, nodeNeiAveDeg2, nodeDeg, degArrSize, degArrNo, degArrVal);
    const NodeType nodeSize = nodeDeg.size(), degSize = degArrSize.size();
    neiAveDeg2.assign(degSize, 0);
    for(NodeType i = 0; i < nodeSize; i++) neiAveDeg2[degArrNo[nodeDeg[i]]] += nodeNeiAveDeg2[i] * nodeWeightOut[i];
    for(NodeType i = 0; i < degSize; i++)
...
int cal_spearman_lkk(double& spearman, const VVLinkType& lkk, const VDouble& deg2ArrVal, const LinkType linkSize)
{
    ...
    if(n * sxx - sx * sx == 0) spearman = 0;
    else spearman = (n * sxy - sx * sx) / (n * sxx - sx * sx);
    return 0;
}
...
int cal_spearman_dir_weight(double& spearman, double& rho, const VVNodeType& p2p, const VVWeightType& vvweight, const double netWeight, const VNodeType& nodeDeg1, MNodeType& degArrNo1, const VDouble& deg2ArrVal1, const VNodeType& nodeDeg2, MNodeType& degArrNo2, const VDouble& deg2ArrVal2, const int weightFlag)
{
    ...
    if(!weightFlag) return cal_spearman_dir(spearman, rho, p2p, netWeight, nodeDeg1, degArrNo1, deg2ArrVal1, nodeDeg2, degArrNo2, deg2ArrVal2);
    double sx = 0., sxx = 0., sy = 0., syy = 0., sxy = 0.;
    for(NodeType i = 0; i < p2p.size(); i++) {
        const NodeType s = p2p[i].size();
        if(s <= 0) continue;
        const double ki = deg2ArrVal1[degArrNo1[nodeDeg1[i]]];
        for(NodeType j = 0; j < s; j++) {
            const double kj = deg2ArrVal2[degArrNo2[nodeDeg2[p2p[i][j]]]], w = vvweight[i][j];
            sx += w * ki;
            sxx += w * ki * ki;
            sy += w * kj;
            syy += w * kj * kj;
...
int cal_spearman_link(double& spearman, VNodeType& link, const VNodeType& nodeDeg, VDouble& deg2ArrVal, MNodeType& degArrNo)
{
    if(deg2ArrVal.size() <= 1){
        spearman = 0;
        return 0;
    ...
    for(LinkType i = 0; i < linkSize; i++) {
        double x = deg2ArrVal[degArrNo[nodeDeg[link[i*2]]]], y = deg2ArrVal[degArrNo[nodeDeg[link[i*2+1]]]];
        sx += x + y;
        sxx += x * x + y * y;
        sxy += x * y;
    }
    if(n * sxx - sx * sx == 0) spearman = 0;
    else spearman = (n * 2. * sxy - sx * sx) / (n * sxx - sx * sx);
    return 0;
}
