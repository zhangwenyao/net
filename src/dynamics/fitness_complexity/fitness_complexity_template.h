...
int network::fitness_complexity::export_2_Mcp(
  ...
  for (size_t c = 0; c < NC; c++) {
    for (size_t p = 0; p < NP; p++) {
      mcp[c][p] =
          (T2)(e[c][p] > 0 && (double)e[c][p] * Qsum >= (double)Qc[c] * Qp[p]
                   ? 1
                   : 0);
    }
  }
...
int network::fitness_complexity::Mcp_2_FC(
  ...
  for (int count = 1; 1; count++) {
    ...
    for (size_t c = 0; c < NC; c++) Fc[c] = iFc[c] < 0 ? -1 : iFc[c] / ifMean;
    for (size_t p = 0; p < NP; p++) Cp[p] = iCp[p] < 0 ? -1 : iCp[p] / icMean;
...
template <typename T>
int network::fitness_complexity::Mcp_2_FC_p2p(
    VDouble& Fc, VDouble& Cp, const std::vector<std::vector<T>>& c2p,
...
template <typename T, typename T2>
int network::fitness_complexity::filter_data2_code2(
    const char* d0File, const char* code0File, const char* codeFile,
    const char* dFile_dir, const char* dFile_dir2, const size_t y1,
    const size_t y2, const size_t y0) {
  std::vector<std::vector<T>> d0, d;
  std::vector<T> d2;
  std::vector<T2> code0, code;
  ...
  d2.resize(code.size());
  for (size_t y = y1; y < y2; ++y) {
    for (size_t i = 0; i < code.size(); ++i) d2[i] = d[i][y - y1];
    _ERR(common::save1((name + std::to_string(y) + dFile_dir2).c_str(), d2,
                       '\n'));
  }
  return 0;
}
