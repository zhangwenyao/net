...
int Spread::init (const Network & net)
{
	...
	if (source.size() != nodeSize || num.size() != nodeSize || nei.size() != nodeSize || stk.size() != nodeSize || prob_rand.size() != kMax + 1) {
		return -1;
	}
...
int Spread::check (const Network & net)
{
	...
	if (head < nodeSize && (SOURCE_HEAD == SOURCE_NULL || SOURCE_HEAD < nodeSize || SOURCE_NULL < nodeSize)) {
		return -1;
	}
	return 0;
}
...
int Spread::saveSpread (const Network &net)
{
	...
	fileName.clear();
	fileName << net.outName << '_' << net.seed << "_spread_stk.txt";	//fileName.str().c_str()
...
int Spread::saveSpread (const Grid& net)
{
	...
	fileName.clear();
	fileName << net.outName << '_' << net.seed << "_spread_num.txt";
	if (::save (fileName.str().c_str(), &num[0], net.NX, net.NY, net.priChar) != 0) return -1;
	fileName.str("");
	fileName.clear();
	fileName << net.outName << '_' << net.seed << "_spread_source.txt";
	if (::save (fileName.str().c_str(), &source[0], net.NX, net.NY, net.priChar) != 0) return -1;
	fileName.str("");
	fileName.clear();
	fileName << net.outName << '_' << net.seed << "_spread_nei.txt";
	if (::save (fileName.str().c_str(), &nei[0], net.NX, net.NY, net.priChar) != 0) return -1;
	return 0;
}
#endif // NET_GRID_SAVE
...
int Spread::spread_same (const Network & net)
{
	...
	time[head] = 0;
	update_nei (net, head);
	for (NodeType step = 1, time0 = 1; step < spreadSize && nei1 < nei0; time0++) {
		...
		for (NodeType iNum = nei1; iNum < nei0_tmp; iNum++) {
			...
			if (r <= prob_rand[iNei]) {  //spread to ni
				...
				for (vector<NodeType>::const_iterator i = net.p2p[ni].begin(); i != net.p2p[ni].end(); i++) {
...
int Spread::spread_diff (const Network & net)
{
	...
	for (NodeType step = 1; step < spreadSize && nei1 < nei0;) {
		NodeType iNum = RAND2() % (nei0 - nei1) + nei1, ni = stk[iNum], iNei = nei[ni], r = RAND2() % net.kMax;
...
inline void Spread::update_nei (const Network & net, const NodeType ni)
{
	...
	nei1++;
	neiCountSize -= nei[ni];
	for (vector<NodeType>::const_iterator itr = net.p2p[ni].begin(); itr != net.p2p[ni].end(); itr++) {
		NodeType nj = *itr;
...
int Spread::saveData (const Network &net)
{
	...
	fileName.str("");
	fileName.clear();
	fileName << net.outName << '_' << net.seed << "_spread_dataTime.txt";	//fileName.str().c_str()
