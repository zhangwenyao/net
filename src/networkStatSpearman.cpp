...
ostream& operator<<(ostream& os, const Stat_spearman& spearman) {
  if (!os || 0 != spearman.save_params(os)) ERROR();
  return os;
}

int Stat_spearman::save_params(ostream& os, const int dirFlag) const {
  if (!os) {
    ERROR();
    return -1;
  }
  ...
  if (dirFlag) {
    ...
    if (STAT_TYPE_DIRAA) {
      os << "--spearman.OutOut\t" << OutOut
        << "\n--spearman.rhoOutOut\t" << rhoOutOut
        << "\n--spearman.InIn\t" << InIn
        << "\n--spearman.rhoInIn\t" << rhoInIn
        << "\n--spearman.InOut\t" << InOut
...
int Stat_spearman::save_params(const char* name, const int dirFlag) const {
...
int Stat_spearman::save_data(const char* name, const int dirFlag, const char priChar, const char priChar2) const {
  if (name == NULL || name[0] == '\0') {
    ERROR();
    return -1;
  }
...
int Stat_spearman::save(const char* name, const int dirFlag, const char priChar, const char priChar2) const {
  if (name == NULL || name[0] == '\0') {
    ERROR();
    return -1;
  }
...
int Stat_spearman::read_params_1(string& s, istream& is) {
  if (!is) {
    ERROR();
    return -1;
  }
...
Stat_spearman& Stat_spearman::clear(void) {
  spearman = 0;
  InIn = 0;
  InOut = 0;
  OutIn = 0;
...
Networks& Networks::stat_spearman(void) {
  ...
  if (!dirFlag) {
    if (!lkk.empty() && !weightFlag)
      cal_spearman_lkk(spearman.spearman, lkk, spearman.deg2ArrVal, linkSize);
    if (!p2p.empty()) {
      cal_nodeNeiAveDeg2_weight(spearman.nodeNeiAveDeg2, p2p, nodeDeg, spearman.deg2ArrVal,
          degArrNo, vvweight, nodeWeight, weightFlag);
      cal_neiAveDeg2_weight(spearman.neiAveDeg2, spearman.nodeNeiAveDeg2, nodeDeg, degArrSize,
          degArrNo, degArrVal, degArrWeight, nodeWeight,
          weightFlag);
      if (!weightFlag) {
        if (lkk.empty())
          cal_spearman(spearman.spearman, p2p, spearman.deg2ArrVal, degArrNo,
              linkSize);
      } else
        cal_spearman_dir_weight(spearman.spearman, spearman.rho,
    ...
    // AllAll
    cal_nodeNeiAveDeg2_AllAll(spearman.nodeNeiAveDeg2, p2p, nodeDeg, spearman.deg2ArrVal,
        degArrNo, vvweight, nodeWeight, weightFlag);
    cal_neiAveDeg2_weight(spearman.neiAveDeg2, spearman.nodeNeiAveDeg2, nodeDeg, degArrSize,
        degArrNo, degArrVal, degArrWeight, nodeWeight,
        weightFlag);
    cal_spearman_dir_weight(spearman.spearman, spearman.rho, p2p,
    ...
    // OutIn
    cal_nodeNeiAveDeg2_weight(spearman.nodeNeiAveDeg2In, p2p, nodeDegIn, spearman.deg2ArrValIn,
        degArrNoIn, vvweight, nodeWeightOut, weightFlag);
    cal_neiAveDeg2_weight(spearman.neiAveDeg2OutIn, spearman.nodeNeiAveDeg2In, nodeDegOut,
        degArrSizeOut, degArrNoOut, degArrValOut,
        degArrWeightOut, nodeWeightOut, weightFlag);
    cal_spearman_dir_weight(
        spearman.OutIn, spearman.rhoOutIn, p2p, vvweight,
        (weightFlag ? netWeightOut : linkSize), nodeDegOut, degArrNoOut,
        spearman.deg2ArrValOut, nodeDegIn, degArrNoIn, spearman.deg2ArrValIn, weightFlag);

    if (STAT_TYPE_DIRAA) {
      // OutOut
      cal_nodeNeiAveDeg2_weight(spearman.nodeNeiAveDeg2Out, p2p, nodeDegOut,
          spearman.deg2ArrValOut, degArrNoOut, vvweight,
          nodeWeightOut, weightFlag);
      cal_neiAveDeg2_weight(spearman.neiAveDeg2OutOut, spearman.nodeNeiAveDeg2Out, nodeDegOut,
          degArrSizeOut, degArrNoOut, degArrValOut,
          degArrWeightOut, nodeWeightOut, weightFlag);
      cal_spearman_dir_weight(
          spearman.OutOut, spearman.rhoOutOut, p2p, vvweight,
          (weightFlag ? netWeightOut : linkSize), nodeDegOut, degArrNoOut,
          spearman.deg2ArrValOut, nodeDegOut, degArrNoOut, spearman.deg2ArrValOut, weightFlag);

      // InIn
      cal_neiAveDeg2_weight(spearman.neiAveDeg2InIn, spearman.nodeNeiAveDeg2In, nodeDegIn,
          degArrSizeIn, degArrNoIn, degArrValIn,
          degArrWeightOut, nodeWeightOut, weightFlag);
      cal_spearman_dir_weight(
          spearman.InIn, spearman.rhoInIn, p2p, vvweight,
          (weightFlag ? netWeightOut : linkSize), nodeDegIn, degArrNoIn,
          spearman.deg2ArrValIn, nodeDegIn, degArrNoIn, spearman.deg2ArrValIn, weightFlag);

      // InOut
      cal_neiAveDeg2_weight(spearman.neiAveDeg2InOut, spearman.nodeNeiAveDeg2Out, nodeDegIn,
          degArrSizeIn, degArrNoIn, degArrValIn,
          degArrWeightOut, nodeWeightOut, weightFlag);
      cal_spearman_dir_weight(
          spearman.InOut, spearman.rhoInOut, p2p, vvweight,
          (weightFlag ? netWeightOut : linkSize), nodeDegIn, degArrNoIn,
          spearman.deg2ArrValIn, nodeDegOut, degArrNoOut, spearman.deg2ArrValOut, weightFlag);
    }
  }
...
Networks& Networks::spearman_degArr_2_deg2ArrVal_weight(void) {
  ...
  if (spearman.deg2ArrVal.empty()) {
    if (!weightFlag) {
      if (0 != degArrVal_2_deg2ArrVal(spearman.deg2ArrVal, degArrSize, degArrVal,
            linkSize, dirFlag)) {
        ERROR();
        runStatus = -1;
        return *this;
      ...
      if (0 != degArrWeight_2_deg2ArrVal(spearman.deg2ArrVal, degArrWeight, netWeight)) {
        ERROR();
        runStatus = -1;
        return *this;
      }
  ...
  if (dirFlag) {       // dirFlag
    if (weightFlag) {  // dirFlag weightFlag
      ...
      if (spearman.deg2ArrValOut.empty() &&
          0 != degArrWeight_2_deg2ArrVal(spearman.deg2ArrValOut, degArrWeightOut,
            netWeightOut)) {
        ERROR();
        runStatus = -1;
        return *this;
      ...
      if (spearman.deg2ArrValIn.empty() &&
          0 != degArrWeight_2_deg2ArrVal(spearman.deg2ArrValIn, degArrWeightIn,
            netWeightIn)) {
        ERROR();
        runStatus = -1;
        return *this;
      ...
      // Out
      if (spearman.deg2ArrValOut.empty() &&
          0 != degArrVal_2_deg2ArrVal(spearman.deg2ArrValOut, degArrSizeOut,
      ...
      // In
      if (spearman.deg2ArrValIn.empty() &&
          0 != degArrVal_2_deg2ArrVal(spearman.deg2ArrValIn, degArrSizeIn, degArrValIn,
            linkSize, dirFlag)) {
        ERROR();
        runStatus = -1;
        return *this;
...
Networks& Networks::spearman_cal_lkkProb_gauss(void) {
  ...
  for (NodeType i = 0; i < degSize; i++) {
    cal_lkkProb_gaussS2(spearman.GaussS2[i], spearman.deg2ArrVal, degArrVal, degArrSize, i, spearman.r0);
  }
  lkkProb.resize(degSize, VDouble(degSize));
  cal_lkkProb_gauss(lkkProb, spearman.GaussS2, spearman.deg2ArrVal, spearman.r0);
  return *this;
}

Networks& Networks::spearman_gauss_new_ranLink(void)  // 随机选点按度之差的相应高斯概率连边，重连时随机选点再选边
{
  // 初始化连边信息
  p2p.resize(nodeSize);
  ...
  if (spearman.deg2ArrVal.empty() &&
      0 != degArrVal_2_deg2ArrVal(spearman.deg2ArrVal, degArrSize, degArrVal, linkSize)) {
    ERROR();
    return *this;
  }
