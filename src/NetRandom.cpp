    ...
    const LinkType tryCount) // 每次直接随机抽取两个点连边
{
  for (LinkType try0 = 0; linkRemain > 0 && remNodeNum.size() > 1;) {
    ...
    std::uniform_int_distribution<NodeType> diss(0, remNodeNum.size() - 2);
    ...
    VNodeType& link, const LinkType tryCount)
{
  for (LinkType try0 = 0; linkRemain > 0 && remNodeNum.size() > 1;) {
    ...
    NodeType i, j;
    while (1) {
      std::uniform_int_distribution<LinkType> dis(0, linkRemain * 2 - 1), dis2(0, linkRemain * 2 - 2);
      ik = dis(rand2);
      i = link[ik];
      jk = dis2(rand2);
      if (jk >= ik)
        jk++;
      j = link[jk];
      if (i != j && rand_double() <= lkkProb[degArrNo[nodeDeg[i]]][degArrNo[nodeDeg[j]]])
        ;
      break;
    }
    if (p2p[i].size() > p2p[j].size()) {
