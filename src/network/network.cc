  ...
  if (net.dirFlag && !net.degArrSumOut.empty()) {
    os << "\n--nodeSizeOut\t"
       << (net.degArrSumOut.back()
              - (net.degArrValOut.front() <= 0 ? net.degArrSizeOut.front()
                                               : 0))
       << "\n--nodeSizeIn\t"
...
network::Network& network::Network::save_deg(const char* name)
{
  ...
  if (!degArrVal.empty())
    if (0
        != common::save1(
            (fn + ".degArrVal.txt").c_str(), degArrVal, priChar)) {
      runStatus = -1;
      ERROR();
    }
  if (!degArrSize.empty())
  ...
  if (!nodesName.empty())
    if (0
        != common::save1(
            (fn + ".nodesName.txt").c_str(), nodesName, priChar)) {
      runStatus = -1;
      ERROR();
    }
  if (!nodesName2.empty())
  ...
  if (weightFlag) {
    ...
    if (!degArrWeight.empty()
        && 0
            != common::save1(
                (fn + ".degArrWeight.txt").c_str(), degArrWeight, priChar)) {
      runStatus = -1;
      ERROR();
    }
  } // weightFlag
  if (dirFlag) {
    ...
    if (!degArrValOut.empty())
      if (0
          != common::save1(
              (fn + ".degArrValOut.txt").c_str(), degArrValOut, priChar)) {
        runStatus = -1;
        ERROR();
      }
    if (!degArrSizeOut.empty())
      if (0
          != common::save1(
              (fn + ".degArrSizeOut.txt").c_str(), degArrSizeOut, priChar)) {
        runStatus = -1;
        ERROR();
      }
    ...
    if (!degArrSizeIn.empty())
      if (0
          != common::save1(
              (fn + ".degArrSizeIn.txt").c_str(), degArrSizeIn, priChar)) {
        runStatus = -1;
        ERROR();
      }
    ...
    if (weightFlag) {
      if (!nodeWeightOut.empty()
          && 0
              != common::save1((fn + ".nodeWeightOut.txt").c_str(),
      ...
      if (!degArrWeightOut.empty()
          && 0
              != common::save1((fn + ".degArrWeightOut.txt").c_str(),
      ...
      if (!nodeWeightIn.empty()
          && 0
              != common::save1((fn + ".nodeWeightIn.txt").c_str(),
      ...
      if (!degArrWeightIn.empty()
          && 0
              != common::save1((fn + ".degArrWeightIn.txt").c_str(),
...
network::Network& network::Network::save_lkk(const char* name)
{
  ...
  case lkk2_lkk2compress:
    if (!lkk2.empty())
      if (0
          != save_lkk2compress(
              (fn + ".lkk2.txt").c_str(), lkk2, priChar2, priChar)) {
        ERROR();
        runStatus = -1;
      }
    break;
...
network::Network& network::Network::save_p2p(const char* name)
{
  ...
  if (!link.empty()
      && 0
          != common::save2(
              (fn + ".link.txt").c_str(), &link[0], linkSize, 2, priChar2)) {
    runStatus = -1;
    ERROR();
  }
  ...
  if (linkMatr.size() > 0) {
    if (0
        != common::save2(
            (fn + ".linkMatr.txt").c_str(), linkMatr, priChar2)) {
      runStatus = -1;
      ERROR();
    }
  }
...
network::Network& network::Network::read_lkk(const char* name, Lkk_type lkk_t)
{
  ...
  case lkk3_lkk3reverse:
    if (0
        != read_lkk3reverse(
            (fn + ".lkk3reverse.txt").c_str(), degArrVal.size() - 1, lkk3)) {
      runStatus = -1;
      ERROR();
      return *this;
    }
...
network::Network& network::Network::read_link_weight(const char* name)
{
  ...
  if (0
      != read_link_weight_0(link, linkSize, vvweight, vvweightIn, fn.c_str(),
          weight_m, weight_n, dirFlag)) {
    runStatus = -1;
    ERROR();
    return *this;
