...
// ******************************************************
int main_func::fitness_complexity::count(int argc, char** argv)
{
  VNodeType cCode, pCode;
  _ERR(common_read1_0(
  ...
  VVLinkType gdps0;
  _ERR(common_read2_0((DIR_DATA0 + "2001-2014.gdps.txt").c_str(), gdps0));
  VVDouble gdpGrows0;
  _ERR(common_read2_0(
  ...
  for (size_t year = YEAR1; year < YEAR2; year++) {
    ...
    VDouble pc, cf, gdp, gdpGrow;
    {
      ...
      _ERR(common_save1((DIR_DATA + to_string(year) + ".gdpGrow.txt").c_str(),
          gdpGrow, '\n'));
