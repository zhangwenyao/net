...
template <typename T> int common::read(std::istream& is, T* a, const size_t n)
{
  _ERR(!is);
  size_t i = 0;
  while (i < n && is >> *a++)
...
template <typename T> int common::read(const char* name, T* a, const size_t n)
{
  std::ifstream is(name);
  if (!is) {
    ERROR("open file ", name);
...
template <typename T> int common::read1(std::istream& is, std::vector<T>& v)
{
  _ERR(!is);
  for (auto& i : v)
    is >> i;
...
template <typename T> int common::read1(const char* name, std::vector<T>& v)
{
  std::ifstream is(name);
  if (!is) {
    ERROR("open file ", name);
