...
// 同一个度分布，不同种子seed、交换概率rho
int main_func::rewire::networks_r_random1(int argc, char** argv)
{
  for (int e = kEMin; e <= kEMax; ++e) {
    SHOW_TIME(cout); // 显示系统时间
    string data_dir = kDataDir + "2^" + to_string(e) + "/", fn_full;
...
#ifdef SPEARMAN_NEI2
      {
        cout << "nodeNeiAveDeg2 mean" << endl;
        string spn
            = save_prename + ".rewire_" + kRhoStrings[i] + ".nodeNeiAveDeg2";
        for (int e = kEMin; e <= kEMax; ++e) {
          cout << "\te\t" << e << endl;
          string prename = kDataDir + "2^" + to_string(e) + "/kMin"
              + to_string(kMin) + "_",
                 sufname
              = ".rewire_" + kRhoStrings[i] + ".nodeNeiAveDeg2.txt";
          _ERR(0
              != statistics::mean<double>(prename.c_str(), sufname.c_str(),
                  kSeedMin, kSeedMax, spn.c_str()));
        }
      }
      {
        cout << "neiAveDeg2 mean" << endl;
        string spn
            = save_prename + ".rewire_" + kRhoStrings[i] + ".neiAveDeg2";
        for (int e = kEMin; e <= kEMax; ++e) {
          cout << "\te\t" << e << endl;
          string prename = kDataDir + "2^" + to_string(e) + "/kMin"
              + to_string(kMin) + "_",
                 sufname = ".rewire_" + kRhoStrings[i] + ".neiAveDeg2.txt";
          _ERR(0
              != statistics::mean<double>(prename.c_str(), sufname.c_str(),
                  kSeedMin, kSeedMax, spn.c_str()));
        }
