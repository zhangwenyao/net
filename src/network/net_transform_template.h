...
int network::linkMatr_2_nodeDeg(std::vector<T>& nodeDeg,
                                const std::vector<std::vector<T2>>& linkMatr) {
  const size_t iSize = linkMatr.size(), jSize = linkMatr[0].size();
  nodeDeg.assign(iSize, 0);
  for (size_t i = 0; i < iSize; i++) {
    for (size_t j = 0; j < jSize; j++) {
      if (linkMatr[i][j] != 0) ++nodeDeg[i];
    }
  }
  return 0;
}
...
int network::lkk_dir_2_nDir(std::vector<std::vector<T>>& lkk, const int dir) {
  ...
  for (NodeType i = 0; i < degSize; i++) {
    ...
    if (dir)
      for (NodeType j = i; j < degSize; ++j) lkk[j][i] += lkk[i][j];
    lkk[i].resize(i + 1);
  }
  return 0;
}
...
int network::lkk_nDir_2_dir(std::vector<std::vector<T>>& lkk, const int dir) {
  ...
  for (NodeType i = 0; i < degSize; i++) {
    ...
    if (dir)
      for (NodeType j = 0; j < lkk[i].size(); j++) lkk[i][j] /= 2;
