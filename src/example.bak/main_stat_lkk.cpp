...
int main(int argc, char** argv)
{
  ...
  for (int e = 5; e <= 25; ++e) {
    ...
    for (int seed = 1; seed <= seed_max; ++seed) {
      ...
      fn2 = net.saveName + ".Min_" + ss.str();
      ofstream os((fn2 + ".lkk3.txt").c_str());
      if (!os) {
        is.close();
        ERROR();
        continue;
      }
      for (long long i, j, n; is >> i >> j >> n;)
        os << i << '\t' << j << '\t' << n << '\n';
      is.close();
      os.close();
      // net.read_params((fn + ".params.txt").c_str());
      // const NodeType degSize = net.degSize;
      // net.degArrVal.resize(degSize, 0);
      // common_read1((fn + ".degArrVal.txt").c_str(), net.degArrVal);
      // net.degArrSize.resize(degSize, 0);
      // common_read1((fn + ".degArrSize.txt").c_str(), net.degArrSize);
    ...
    // cout << "2e" << e << "\t" << pmean << "\t" << perr << endl;
    // po << "2e" << e << "\t" << pmean << "\t" << perr << "\n";

    // tmean /= seed_max;
    // terr = sqrt(terr / seed_max - tmean * tmean);
    // cout << "2e" << e << "\t" << tmean << "\t" << terr << endl;
    // to << "2e" << e << "\t" << tmean << "\t" << terr << "\n";

    // smean /= seed_max;
    // serr = sqrt(serr / seed_max - smean * smean);
    // cout << "2e" << e << "\t" << smean << "\t" << serr << endl;
    // so << "2e" << e << "\t" << smean << "\t" << serr << "\n";
  }
