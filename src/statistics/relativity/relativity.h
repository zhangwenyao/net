...
namespace relativity {
  ...
  int cal_relativity_lkk3(double& relativity, const VLkk3LinkType& lkk3,
      const VNodeType& degArrVal, const LinkType& linkSize,
      const double alpha);
  int cal_relativity_p2p(double& relativity, const VVNodeType& p2p,
      const LinkType linkSize, const double alpha);
  int cal_relativity_link(double& relativity, const VNodeType& link,
      const VNodeType& nodeDeg, const double alpha);
  int alphas_sum(const char* prename, const char* sufname, const int seed_min,
      const int seed_max, VDouble& sx, VDouble& sx2, size_t& n);
}
}
