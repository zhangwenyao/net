...
int Stat_spearman::save_data(const char* name, const int dirFlag,
...
int Stat_spearman::save(const char* name, const int dirFlag,
    const char priChar, const char priChar2) const
{
  if (name == NULL || name[0] == '\0') {
    ERROR();
...
Networks& Networks::stat_spearman(void)
{
  ...
  do {
    if (!dirFlag) {
      ...
      if (!p2p.empty()) {
        cal_nodeNeiAveDeg2_weight(spearman.nodeNeiAveDeg2, p2p, nodeDeg,
            spearman.deg2ArrVal, degArrNo, vvweight, nodeWeight, weightFlag);
        cal_neiAveDeg2_weight(spearman.neiAveDeg2, spearman.nodeNeiAveDeg2,
            nodeDeg, degArrSize, degArrNo, degArrVal, degArrWeight, nodeWeight,
            weightFlag);
        if (!weightFlag) {
          if (lkk.empty())
          ...
          cal_spearman_dir_weight(spearman.spearman, spearman.rho, p2p,
              vvweight, netWeight, nodeDeg, degArrNo, spearman.deg2ArrVal,
              nodeDeg, degArrNo, spearman.deg2ArrVal, weightFlag);
      }

    } else { // dirFlag
      // AllAll
      cal_nodeNeiAveDeg2_AllAll(spearman.nodeNeiAveDeg2, p2p, nodeDeg,
          spearman.deg2ArrVal, degArrNo, vvweight, nodeWeight, weightFlag);
      cal_neiAveDeg2_weight(spearman.neiAveDeg2, spearman.nodeNeiAveDeg2,
          nodeDeg, degArrSize, degArrNo, degArrVal, degArrWeight, nodeWeight,
          weightFlag);
      cal_spearman_dir_weight(spearman.spearman, spearman.rho, p2p, vvweight,
          (weightFlag ? netWeightOut : linkSize), nodeDeg, degArrNo,
          spearman.deg2ArrVal, nodeDeg, degArrNo, spearman.deg2ArrVal,
          weightFlag);

      // OutIn
      cal_nodeNeiAveDeg2_weight(spearman.nodeNeiAveDeg2In, p2p, nodeDegIn,
          spearman.deg2ArrValIn, degArrNoIn, vvweight, nodeWeightOut,
          weightFlag);
      cal_neiAveDeg2_weight(spearman.neiAveDeg2OutIn,
          spearman.nodeNeiAveDeg2In, nodeDegOut, degArrSizeOut, degArrNoOut,
          degArrValOut, degArrWeightOut, nodeWeightOut, weightFlag);
      cal_spearman_dir_weight(spearman.OutIn, spearman.rhoOutIn, p2p, vvweight,
          (weightFlag ? netWeightOut : linkSize), nodeDegOut, degArrNoOut,
          spearman.deg2ArrValOut, nodeDegIn, degArrNoIn, spearman.deg2ArrValIn,
          weightFlag);
      ...
      if (STAT_TYPE_DIRAA) {
        ...
        cal_nodeNeiAveDeg2_weight(spearman.nodeNeiAveDeg2Out, p2p, nodeDegOut,
            ...
            weightFlag);
        cal_neiAveDeg2_weight(spearman.neiAveDeg2OutOut,
            spearman.nodeNeiAveDeg2Out, nodeDegOut, degArrSizeOut, degArrNoOut,
            degArrValOut, degArrWeightOut, nodeWeightOut, weightFlag);
        cal_spearman_dir_weight(spearman.OutOut, spearman.rhoOutOut, p2p,
            vvweight, (weightFlag ? netWeightOut : linkSize), nodeDegOut,
            degArrNoOut, spearman.deg2ArrValOut, nodeDegOut, degArrNoOut,
            spearman.deg2ArrValOut, weightFlag);

        // InIn
        cal_neiAveDeg2_weight(spearman.neiAveDeg2InIn,
            spearman.nodeNeiAveDeg2In, nodeDegIn, degArrSizeIn, degArrNoIn,
            degArrValIn, degArrWeightOut, nodeWeightOut, weightFlag);
        cal_spearman_dir_weight(spearman.InIn, spearman.rhoInIn, p2p, vvweight,
        ...
        // InOut
        cal_neiAveDeg2_weight(spearman.neiAveDeg2InOut,
            spearman.nodeNeiAveDeg2Out, nodeDegIn, degArrSizeIn, degArrNoIn,
            degArrValIn, degArrWeightOut, nodeWeightOut, weightFlag);
        cal_spearman_dir_weight(spearman.InOut, spearman.rhoInOut, p2p,
            vvweight, (weightFlag ? netWeightOut : linkSize), nodeDegIn,
            degArrNoIn, spearman.deg2ArrValIn, nodeDegOut, degArrNoOut,
            spearman.deg2ArrValOut, weightFlag);
      }
    }
  } while (0);
...
Networks& Networks::spearman_degArr_2_deg2ArrVal_weight(void)
{
  ...
  if (spearman.deg2ArrVal.empty()) {
    if (!weightFlag) {
      if (0 != degArrVal_2_deg2ArrVal(spearman.deg2ArrVal, degArrSize,
      ...
      if (0 != degArrWeight_2_deg2ArrVal(
  ...
  if (dirFlag) {      // dirFlag
    if (weightFlag) { // dirFlag weightFlag
      ...
      // In
      if (spearman.deg2ArrValIn.empty()
          && 0 != degArrVal_2_deg2ArrVal(spearman.deg2ArrValIn, degArrSizeIn,
