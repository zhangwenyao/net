...
Network& Network::save_deg(const char* name)
{
  ...
  if (weightFlag) {
    ...
    if (!degArrWeight.empty()
        && 0
            != common_save1((fn + ".degArrWeight.txt").c_str(), degArrWeight,
                   priChar)) {
      runStatus = -1;
      ERROR();
    }
  ...
  if (dirFlag) {
    ...
    if (!degArrSizeOut.empty())
      if (0
          != common_save1((fn + ".degArrSizeOut.txt").c_str(), degArrSizeOut,
                 priChar)) {
        runStatus = -1;
        ERROR();
      }
    ...
    if (weightFlag) {
      if (!nodeWeightOut.empty()
          && 0
              != common_save1((fn + ".nodeWeightOut.txt").c_str(),
      ...
      if (!degArrWeightOut.empty()
          && 0
              != common_save1((fn + ".degArrWeightOut.txt").c_str(),
      ...
      if (!nodeWeightIn.empty()
          && 0
              != common_save1((fn + ".nodeWeightIn.txt").c_str(),
      ...
      if (!degArrWeightIn.empty()
          && 0
              != common_save1((fn + ".degArrWeightIn.txt").c_str(),
...
Network& Network::save_p2p(const char* name)
{
  ...
  if (!link.empty()
      && 0
          != common_save2((fn + ".link.txt").c_str(), &link[0], linkSize, 2,
                 priChar2)) {
    runStatus = -1;
    ERROR();
  }
  ...
  if (weightFlag) {
    ...
    if (dirFlag) {
      ...
      if (!vvweightIn.empty()
          && 0
              != common_save2((fn + ".vvweightIn.txt").c_str(), vvweightIn,
                     priChar2)) {
        runStatus = -1;
        ERROR();
      }
