...
// ******************************************************
network::pearson::Pearson::Pearson(void)
    : pearson(0)
    , No(0)
    , NoInIn(0)
    , NoInOut(0)
...
ostream& operator<<(ostream& os, const network::pearson::Pearson& pearson)
{
...
int network::pearson::Pearson::save_params(
...
int network::pearson::Pearson::save_data(const char* name, const int dirFlag,
...
int network::pearson::Pearson::save(const char* name, const int dirFlag,
    const char priChar, const char priChar2) const
{
  if (name == NULL || name[0] == '\0') {
    ERROR();
...
int network::pearson::Pearson::read_params_1(string& s, istream& is)
{
  if (!is) {
    ERROR();
    return -1;
...
network::pearson::Pearson& network::pearson::Pearson::clear(void)
{
  pearson = 0;
  InIn = 0;
  InOut = 0;
  ...
  do {
    if (!dirFlag) {
      if (!lkk.empty() && !weightFlag) {
        network::pearson::cal_pearson_lkk(
      ...
      if (!p2p.empty()) {
        ...
        if (!weightFlag) {
          ...
          network::pearson::cal_pearson_dir_weight(pearson.pearson,
              pearson.rho, p2p, vvweight, netWeight, nodeDeg, nodeDeg,
              weightFlag);
      }
