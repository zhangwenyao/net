...
// ******************************************************
int network::modularity::save_moduStk(ostream& os, const VNodeType& moduStk,
    const VRNodeType& moduRange, const char c)
{
  ...
  for (NodeType i = 0; i < size;) {
    save(os, &moduStk[moduRange[i].start],
...
int network::modularity::save_moduStk(const VNodeType& moduStk,
    const VRNodeType& moduRange, const char* name, const char c)
{
...
// ******************************************************
int network::modularity::modularity_move(const NodeType a, const NodeType b,
    VNodeType& moduStk, VNodeType& moduNum,
    VRNodeType& moduRange) // b组移到a组后面
{
  if (a == b || moduRange[a].end == moduRange[b].start)
    return 0;
...
int network::modularity::modularity_merge(const NodeType a, const NodeType b,
    VNodeType& moduVal, VNodeType& moduStk, VNodeType& moduNum,
    VRNodeType& moduRange) // b组移到a组后面、合并
{
  ...
  // 更新的组号
  for (NodeType i = r2.start; i < r2.end; i++)
    moduVal[moduStk[i]] = a1;
...
int network::modularity::moduStk_sort(
    VNodeType& moduStk, VRNodeType& moduRange, VNodeType& moduNum)
{
  ...
  for (NodeType i = 0; i < moduStk.size(); i++)
    moduNum[moduStk[i]] = i;
  return 0;
}

// ******************************************************
int network::modularity::cal_modularity(VNodeType& moduVal,
    VNodeType& moduStk, VNodeType& moduNum, VRNodeType& moduRange,
    const VVNodeType& p2p, const VVNodeType& p2pIn, const int dirFlag)
{
  ...
  for (NodeType pos = 0, mEnd = 1; mEnd < nodeSize; pos++) {
    ...
    if (dirFlag) {
      for (VNodeTypeCItr itr = p2pIn[i].begin(); itr != p2pIn[i].end();
           itr++) {
        const NodeType j = *itr;
        if (moduVal[j] == nodeSize) {
          moduVal[j] = moduSize;
...
int network::modularity::update_modularity_iNode(const NodeType i,
    VNodeType& moduVal, VNodeType& moduStk, VNodeType& moduNum,
    VRNodeType& moduRange, const VVNodeType& p2p, const VVNodeType& p2pIn,
    const int dirFlag)
{
  ...
  while (pos < mEnd) { // 搜索节点i所在分组节点
    ...
    for (VNodeTypeCItr itr = p2p[ii].begin(); itr != p2p[ii].end(); itr++) {
      ...
      if (moduVal[j] != iVal) { // 出现待合并分组
        modularity_merge(
            iVal, moduVal[j], moduVal, moduStk, moduNum, moduRange);
  ...
  if (mEnd < moduRange[i].end) { // 出现新组
    ...
    for (NodeType pos = mEnd, ms = moduRange.size() - 1;
         pos < moduRange[i].end; pos++)
      moduVal[moduStk[pos]] = ms;
    moduRange[i].end = mEnd;
    update_modularity_iNode(moduStk[mEnd], moduVal, moduStk, moduNum,
        moduRange, p2p, p2p, dirFlag); // 继续更新分划新组
  }
...
// ******************************************************
int network::modularity::cal_moduLKK(VVLinkType& moduLKK,
    const NodeType moduSize, const VNodeType& moduVal, const VVNodeType& p2p,
    const int dirFlag)
{
  moduLKK.resize(moduSize);
  moduLKK[0].assign(moduSize, 0);
  for (NodeType i = 1; i < moduSize; i++)
    moduLKK[i] = moduLKK[0];
...
int network::modularity::cal_moduCoef(
    double& moduCoef, const VVLinkType& moduLKK, const int dirFlag)
{
...
int network::modularity::cal_moduNodeCoef(VDouble& moduNodeCoef,
    const NodeType moduSize, const VNodeType& moduVal, const VVNodeType& p2p)
{
  const NodeType nodeSize = p2p.size();
  VNodeType s;
