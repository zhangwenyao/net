    ...
    double& dt, const double lambda)
{
  ...
  dt = 1. / all_i;
  if (rand_double(rand2) * all_i <= N_i) {
    // recover
    ...
    const NodeType degMax)
{
  ...
  if (N_i[M] <= 0) { // all recovered
    ...
    t[M] += dt;
    if (rand_double(rand2) < p * dt) { // renew status
      NodeType i = dis(rand2);
      statusSN[i] = statusSN[M];
      SN[i] = SN[M];
      N_i[i] = N_i[M];
    ...
    const NodeType degMax)
{
  ...
  for (double t2; t_av > 0;) {
    ...
    double t0 = t[M];
    if (0 == SIS_update2(statusSN, SN, N_i, NDeg_i, t, t2, p, lambda, p2p, degMax)) {
      t_av -= t[M] - t0;
      nNum++;
      nSum += N_i[M];
...
int act_SIS_tau(double& tau, VVNodeType& statusSN, VVNodeType& SN,
    ...
    LinkType& n2Sum, NodeType& nNum, const double p,
    const double lambda, const VVNodeType& p2p,
    const NodeType degMax)
{
  const unsigned M = N_i.size() - 1;
  while (1) {
    if (0 != SIS_update2(statusSN, SN, N_i, NDeg_i, t, tau, p, lambda, p2p,
                 degMax)
        || N_i[M] >= p2p.size() / 2)
      break;
    nNum++;
