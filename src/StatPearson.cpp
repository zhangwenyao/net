...
int cal_pearson_dir(double& pearson, double& ps_rho, const VVNodeType& p2p, const LinkType linkSize, const VNodeType& nodeDegOut, const VNodeType& nodeDegIn)
{
    ...
    if(linkSize * sxx - sx * sx == 0){
        ...
        ps_rho = (linkSize * sxy - sx * sy) / (linkSize * sxx - sx * sx);
    }
    return 0;
}
...
{
    if(!weightFlag) return cal_pearson_dir(pearson, ps_rho, p2p, netWeight, nodeDegOut, nodeDegIn);
    double sx = 0., sxx = 0., sy = 0., syy = 0., sxy = 0.;
    for(NodeType i = 0; i < p2p.size(); i++) {
        if(p2p[i].empty()) continue;
    ...
    if(netWeight * sxx - sx * sx == 0){
        ...
        ps_rho = (netWeight * sxy - sx * sy) / (netWeight * sxx - sx * sx);
    }
    return 0;
}
...
int cal_pearsonNo(double& pearsonNo, const VVNodeType& p2p, const LinkType linkSize)
{
    ...
    if(n * sxx - sx * sx == 0) pearsonNo = 0;
    else pearsonNo = (n * sxy - sx * sx) / (n * sxx - sx * sx);
    return 0;
}
