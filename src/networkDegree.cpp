...
{
  // 生成度分布概率
  poisson_cal_degArr(net.params_degree.poisson_p, net.degArrVal, net.degArrProb,
                     net.kMin, net.kMax, net.nodeSize);
  net.nodeDeg.resize(net.nodeSize);
  random_new_randKArr(&net.nodeDeg[0], (size_t)net.nodeSize, &net.degArrProb[0],
                      &net.degArrVal[0],
                      net.degArrVal.size());  // 调用通用随机函数
  // 修正度序列使总数为偶数
  if (nodeDeg_2_linkSize(net.linkSize, net.nodeDeg) != 0) {
    fix_nodeDeg(net.nodeDeg, net.degArrProb, net.degArrVal, net.linkSize);
    net.linkSize /= 2;
  }
  sort(net.nodeDeg.begin(),
       net.nodeDeg.end());  // 调用系统函数sort，对节点按度从小到大排序
  nodeDeg_2_degArr_Sort(net.nodeDeg, net.degArrVal, net.degArrSize,
                        net.degArrSum);
  return 0;
}
...
int net_power_check_params(Network& net) {
  if (net.kMin > net.kMax || net.kMax >= net.nodeSize ||
      ...
      net.params_degree.power_gamma < 0)
    return -1;
  return 0;
}
...
int net_deg_power(Network& net) {
  // 生成度分布概率
  power_cal_degArr(net.params_degree.power_gamma, net.degArrVal, net.degArrProb,
                   net.kMin, net.kMax);
  net.nodeDeg.resize(net.nodeSize);
  random_new_randKArr(&net.nodeDeg[0], (size_t)net.nodeSize, &net.degArrProb[0],
                      &net.degArrVal[0],
