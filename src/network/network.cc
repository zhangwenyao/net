...
network::Network& network::Network::clear_deg(void)
{
  ...
  nodesName.clear(), nodesName2.clear(); // [nodeSize]   �ڵ��ӳ���ϵ
  degArrWeight.clear();
...
network::Network& network::Network::save_deg(const char* name)
{
  ...
  if (weightFlag) {
    ...
    if (!degArrWeight.empty()
        && 0
            != common::save1((fn + ".degArrWeight.txt").c_str(), degArrWeight,
                   priChar)) {
      runStatus = -1;
      ERROR();
    }
  ...
  if (dirFlag) {
    ...
    if (!degArrSizeOut.empty())
      if (0
          != common::save1((fn + ".degArrSizeOut.txt").c_str(), degArrSizeOut,
                 priChar)) {
        runStatus = -1;
        ERROR();
      }
...
network::Network& network::Network::save_p2p(const char* name)
{
  ...
  if (weightFlag) {
    ...
    if (dirFlag) {
      ...
      if (!vvweightIn.empty()
          && 0
              != common::save2((fn + ".vvweightIn.txt").c_str(), vvweightIn,
                     priChar2)) {
        runStatus = -1;
        ERROR();
      }
