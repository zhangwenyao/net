...
// ******************************************************
int network::similarity::cal_similarity(VVDouble& simiNodeCoef,
    VVDouble& simiEdgeCoef, const VNodeType& link1, const VNodeType& link2,
    const int dirFlag)
{
  ...
  simiNodeCoef = x;
  y.resize(l1);
  for (NodeType i = 0; i < l1; ++i)
    y[i].assign(l2, 1);
  simiEdgeCoef = y;
  for (const double d1 = 1 - delta, d2 = 1 + delta; x2Sum0 > 0 && y2Sum0 > 0
       && (d1 * x2Sum0 > x2Sum || x2Sum > d2 * x2Sum0 || d1 * y2Sum0 > y2Sum
              || y2Sum > d2 * y2Sum0);) {
    x2Sum0 = x2Sum;
    y2Sum0 = y2Sum;
    cal_similarity_update(simiNodeCoef, simiEdgeCoef, link1, link2, x, x2Sum,
        y, y2Sum, dirFlag);
  }
  return 0;
}

int network::similarity::cal_similarity_update(VVDouble& simiNodeCoef,
    VVDouble& simiEdgeCoef, const VNodeType& link1, const VNodeType& link2,
    VVDouble& x, double& x2Sum, VVDouble& y, double& y2Sum, const int dirFlag)
{
  const NodeType n1 = simiNodeCoef.size(), n2 = simiNodeCoef[0].size();
  const LinkType l1 = link1.size() / 2, l2 = link2.size() / 2;
