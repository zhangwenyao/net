...
int nodeDeg_2_degArr(const VNodeType& nodeDeg, VNodeType& degArrVal, VNodeType& degArrSize, VNodeType& degArrSum)
{
...
int nodeDeg_2_degArr_Sort(const VNodeType& nodeDeg, VNodeType& degArrVal, VNodeType& degArrSize, VNodeType& degArrSum)
{
    const NodeType nodeSize = nodeDeg.size();
    degArrVal.clear();
    degArrSize.clear();
    ...
    for(NodeType i = 1, di = nodeDeg[0]; i < nodeSize; i++) {
        if(nodeDeg[i] == di) degArrSize.back()++;
        else {
...
int degArr_2_nodeSize(NodeType& nodeSize, const VNodeType& degArrSize)
{
    ...
    for(VNodeTypeCItr i = degArrSize.begin(); i != degArrSize.end(); i++) nodeSize += *i;
    return 0;
}

int degArr_2_nodeDeg(VNodeType& nodeDeg, const VNodeType& degArrVal, const VNodeType& degArrSize)
{
    if(!nodeDeg.empty() || degArrVal.empty() || degArrVal.size() != degArrSize.size()){
        ERROR();
        return -1;
    }
    for(NodeType i = 0; i < degArrVal.size(); i++){
        for(NodeType s = 0, t = degArrVal[i]; s < degArrSize[i]; s++) nodeDeg.push_back(t);
    }
    return 0;
}
...
int degArr_2_linkSize(LinkType& linkSize, const VNodeType& degArrVal, const VNodeType& degArrSize, const int dirFlag)
{
    linkSize = 0;
    const NodeType degSize = degArrVal.size();
    for(NodeType i = 0; i < degSize; i++) linkSize += (LinkType)degArrVal[i] * degArrSize[i];
    if(!dirFlag){
        if(linkSize % 2 != 0) return -1;
        else linkSize /= 2;
    }
    return 0;
}

int degArr_2_linkSum(VLinkType& linkSum, const VNodeType& degArrVal, const VNodeType& degArrSize)
{
    const NodeType degSize = degArrVal.size();
    linkSum.resize(degSize);
    LinkType sum = 0;
    for(NodeType i = 0; i < degSize; i++) linkSum[i] = sum += (LinkType)degArrVal[i] * degArrSize[i];
    return 0;
}

int nodeDeg_2_linkSize(LinkType &linkSize, const VNodeType& nodeDeg, const int dirFlag)
{
    linkSize = 0;
    for(VNodeTypeCItr i = nodeDeg.begin(); i != nodeDeg.end(); i++) linkSize += *i;
    if(!dirFlag){
        if(linkSize % 2 != 0){
            return -1;
        } else linkSize /= 2;
...
int check_nodeDeg0(const VNodeType& nodeDeg)
{
    ...
    for(VNodeTypeCItr i = nodeDeg.begin(); i != nodeDeg.end(); i++) if(*i <= 0) return 1;
    return 0;
}

int fix_nodeDeg(VNodeType& nodeDeg, const VDouble& degArrProb, const VNodeType& degArrVal, LinkType& linkSize)
{
...
int p2p_2_lkk(VVLinkType& lkk, const VVNodeType& p2p, const VNodeType& degNum, const NodeType degSize)
{
    lkk.clear();
...
int p2p_2_lkk_dir(VVLinkType& lkkOutIn, const VVNodeType& p2p, const VNodeType& nodeDegOut, const VNodeType& nodeDegIn, MNodeType& degArrNoOut, MNodeType& degArrNoIn, const NodeType degOutSize, const NodeType degInSize)
{
...
int p2p_2_lkk(VVLinkType& lkk, const VVNodeType& p2p, MNodeType& degArrNo, const NodeType degSize)
{
    lkk.clear();
    if(degSize <= 0)    return 0;
    lkk.resize(degSize);
...
int p2p_2_lkk_noDir(VVLinkType& lkk, const VVNodeType& p2p, MNodeType& degArrNo, const VNodeType& nodeDeg, const NodeType degSize)
{
    lkk.clear();
    if(degSize <= 0)    return 0;
    lkk.resize(degSize);
...
int link_2_p2p(VVNodeType& p2p, const VNodeType& link, VVNodeType& p2pIn, NodeType& nodeSize, const int dirFlag)
{
    ...
    for(LinkType n = 0; n < link.size(); n += 2){
        ...
        p2p[i].push_back(j);
        if(dirFlag) p2pIn[j].push_back(i);
        else p2p[j].push_back(i);
    }
    return 0;
}
...
int link_2_lkk(VVLinkType& lkk, const VNodeType& link, const VNodeType& nodeDeg, MNodeType& degArrNo, const LinkType linkSize, const NodeType degSize)
{
    lkk.resize(degSize);
    lkk[0].assign(degSize, 0);
    for(NodeType i = 1; i < degSize; i++) lkk[i] = lkk[0];
    for(LinkType i = 0; i < linkSize; i++) {
        const NodeType x = degArrNo[nodeDeg[link[i*2]]], y = degArrNo[nodeDeg[link[i*2+1]]];
        lkk[x][y]++;
        lkk[y][x]++;
    }
    return 0;
...
int vvweight_2_nodeWeight(VWeightType& nodeWeight, const VVNodeType& p2p, const VVWeightType& vvweight, WeightSumType& netWeight)
{
...
int nodeDegIO_2_nodeDeg(VNodeType& nodeDeg, const VNodeType& nodeDegOut, const VNodeType& nodeDegIn)
{
    ...
    if(nodeDegOut.size() >= nodeDegIn.size()){
        ...
        nodeDeg = nodeDegIn;
        for(NodeType i = 0; i < nodeDegOut.size(); i++) nodeDeg[i] += nodeDegOut[i];
    }
    return 0;
}

int nodeWeightIO_2_nodeWeight(VWeightType& nodeWeight, const VWeightType& nodeWeightOut, const VWeightType& nodeWeightIn)
{
    if(nodeWeightOut.size() >= nodeWeightIn.size()){
        nodeWeight = nodeWeightOut;
        for(NodeType i = 0; i < nodeWeightIn.size(); i++) nodeWeight[i] += nodeWeightIn[i];
    }else{
        nodeWeight = nodeWeightIn;
        for(NodeType i = 0; i < nodeWeightOut.size(); i++) nodeWeight[i] += nodeWeightOut[i];
    }
    return 0;
}

int nodeWeight_2_degArrWeight(VWeightSumType& degArrWeight, const VWeightSumType& nodeWeight, const VNodeType& nodeDeg, MNodeType& degArrNo, const NodeType degSize)
{
...
int degArrWeight_2_degArrWeightSum(VWeightSumType& degArrWeightSum, const VWeightSumType& degArrWeight)
{
...
int fix_p2p(VVNodeType& p2p, const int dirFlag)
{
    ...
    if(!dirFlag){   // 无向网络相互连接
        for(NodeType i = 0; i < p2p.size(); i++){
            for(VNodeTypeCItr iItr = p2p[i].begin(), jItr; iItr != p2p[i].end(); iItr++){
...
int sort_link_betwEdge(VNodeType& link, VVDouble& betwEdge)
{
    ...
    while(!st.empty()){
        ...
        double pivot = betwEdge[*(start + (end - start) / 2)][*(start + (end - start) / 2 + 1)];
        if(betwEdge[*start][start[1]] > pivot) pivot = betwEdge[*start][start[1]];
        else if(betwEdge[*end][end[1]] < pivot) pivot = betwEdge[*end][end[1]];
        while(start < end){
        ...
        while(start > range.start && betwEdge[*start][start[1]] >= pivot) start -= 2;
        while(end < range.end && betwEdge[*end][end[1]] <= pivot) end += 2;
        if(range.start < start) st.push(Common_RangeP<PNodeType>(range.start, start));
...
int addLink_linkMatrC_proNode(VVChar& linkMatrC, LinkType& linkSize, const double p)	// 所有点按概率p连边
{
    const NodeType nodeSize = linkMatrC.size();
...
int addLink_linkMatr_ranNode(VVDistType& linkMatr, LinkType& linkRemain)	// 每次直接随机抽取两个点连边
{
    const NodeType nodeSize = linkMatr.size();
    while(linkRemain > 0) {
        NodeType i = RAND2() % nodeSize, j = RAND2() % (nodeSize - 1);
...
int addLink_linkMatr_proNode(VVDistType& linkMatr, LinkType& linkSize, const double p)	// 所有点按概率p连边
{
    const NodeType nodeSize = linkMatr.size();
    linkMatr.resize(nodeSize);
...
int addLink_linkMatrC_ranNode2(VVChar& linkMatrC, VNodeType& nodeDeg, VNodeType& p2pSize, VNodeType& remPoiNum, LinkType& linkRemain, const LinkType tryCount)	// 每次直接随机抽取两个点连边
{
...
int addLink_linkMatr_ranNode2(VVDistType& linkMatr, VNodeType& nodeDeg, VNodeType& p2pSize, VNodeType& remPoiNum, LinkType& linkRemain, const LinkType tryCount)	// 每次直接随机抽取两个点连边
{
    for(LinkType try0 = 0; linkRemain > 0 && remPoiNum.size() > 1;) {
        NodeType ik = RAND2() % remPoiNum.size();
        NodeType i = remPoiNum[ik];
...
int addLink_p2p_ranNode0(VVNodeType& p2p, VNodeType& link, VNodeType& nodeDeg, VNodeType& remPoiNum, LinkType &linkRemain, const LinkType tryCount)
{
    for(LinkType try0 = 0; linkRemain > 0 && remPoiNum.size() > 1;) {
...
int addLink_p2p_proDeg(VVNodeType& p2p, VNodeType& link, const VNodeType& nodeDeg, const NodeType kMax, VNodeType& remPoiNum, LinkType &linkRemain, const LinkType tryCount)
{
...
int addLink_p2p_ranNode(VVNodeType& p2p, VNodeType& nodeDeg, VNodeType& remPoiNum, LinkType &linkRemain, const LinkType tryCount)
{
    for(LinkType try0 = 0; linkRemain > 0 && remPoiNum.size() > 1;) {
        NodeType ik = RAND2() % remPoiNum.size(), i;
        i = remPoiNum[ik];
...
int addLink_p2p_ranLink(VVNodeType& p2p, VNodeType& nodeDeg, NodeType &remPoiSize, LinkType &linkRemain, VNodeType& link, const LinkType tryCount)
{
...
int addLink_p2p_ranLink_lkkProb(VVNodeType& p2p, const VNodeType& nodeDeg, VVDouble& lkkProb, MNodeType& degArrNo, NodeType &remPoiSize, LinkType &linkRemain, VNodeType& link, const LinkType tryCount)
{
    for(LinkType try0 = 0; linkRemain > 0 && remPoiSize > 1;) {
        LinkType ik, jk;
        NodeType i, j;
        while(1) {
            ...
            if(jk >= ik) jk++;
            j = link[jk];
            if(i != j && RAND2() <= lkkProb[degArrNo[nodeDeg[i]]][degArrNo[nodeDeg[j]]] * RAND2_MAX) break;
        }
        if(p2p[i].size() > p2p[j].size()) {
            NodeType k = ik;
            ik = jk;
...
int delLink_p2p_ranLink(VVNodeType& p2p, VNodeType& nodeDeg, VNodeType& remPoiNum, LinkType &linkRemain, const LinkType linkSize, VNodeType& link, LinkType delCount)
{
...
int delLink_linkMatrC_ranNode(VVChar& linkMatrC, const NodeType nodeSize, VNodeType& nodeDeg, VNodeType& p2pSize, VNodeType& remPoiNum, LinkType &linkRemain, const LinkType linkSize, LinkType delCount)	// 随机选点删边
{
    while(delCount > 0 && linkRemain < linkSize) {
        NodeType i;
        do {
...
int delLink_p2p_ranNode(VVNodeType& p2p, const NodeType nodeSize, const VNodeType& nodeDeg, VNodeType& remPoiNum, NodeType &remPoiSize, LinkType &linkRemain, const LinkType linkSize, LinkType delCount)	// 随机选点删边
{
    while(delCount > 0 && linkRemain < linkSize) {
        NodeType i;
        do {
...
int delLink_p2p_ranNode1(VVNodeType& p2p, const NodeType nodeSize, VNodeType& nodeDeg, VNodeType& remPoiNum, NodeType &remPoiSize, LinkType &linkRemain, const LinkType linkSize, LinkType delCount)	// 随机选点再选边删除
{
    while(delCount > 0 && linkRemain < linkSize) {
        NodeType i;
        do {
...
int sort_link_greater_all(VNodeType& link, const VNodeType& p2pSize)
{
    ...
    for(char flag = 1; flag; p0 -= 2){
        ...
        for(NodeType *p = &link[0]; p < p0; p += 2) {
            if(p2pSize[*p] > p2pSize[p[2]] || (p2pSize[*p] == p2pSize[p[2]] && p2pSize[p[1]] > p2pSize[p[3]])) {
                NodeType t = *p;
                *p = p[1];
                p[1] = t;
                t = p[1];
...
int exchange_linkC_deg_same(VVChar& linkMatrC, VNodeType& link, const VNodeType& p2pSize, LinkType count)
{
    const LinkType linkSize = link.size() / 2;
    ...
    while(count > 0) {
        ...
        NodeType pi1 = p2pSize[i1], pi2 = p2pSize[i2], pj1 = p2pSize[j1], pj2 = p2pSize[j2];
        if(pi1 >= pj2 || pj1 >= pi2 || i1 == j1 || i2 == j2) continue;
        if(pi1 != pj1 && pi2 != pj2 && ((pj1 < pi1 && pi2 < pj2) || (pi1 < pj1 && pj2 < pi2)) && (linkMatrC[i1][j2 >> 3] & (1 << (j2 & 7))) == 0 && (linkMatrC[i2][j1 >> 3] & (1 << (j1 & 7))) == 0) {
            // i1-i2
        ...
        if((linkMatrC[i1][j1 >> 3] & (1 << (j1 & 7))) || (linkMatrC[i2][j2 >> 3] & (1 << (j2 & 7)))) continue;
        // i1-i2
        linkMatrC[i1][i2 >> 3] ^= 1 << (i2 & 7);
        linkMatrC[i2][i1 >> 3] ^= 1 << (i1 & 7);
        // j1-j2
...
int exchange_linkC_degDiff(VVChar& linkMatrC, VNodeType& link, const VNodeType& p2pSize, LinkType count)
{
    const LinkType linkSize = link.size() / 2;
    if(linkSize < 2) return -1;
    while(count > 0) {
        ...
        NodeType i1 = link[ik], i2 = link[ik | 1], j1 = link[jk], j2 = link[jk | 1];
        NodeType pi1 = p2pSize[i1], pi2 = p2pSize[i2], pj1 = p2pSize[j1], pj2 = p2pSize[j2];
        if(i1 == j2 || i2 == j1 || (pi1 <= pj1 && pj2 <= pi2) || (pj1 <= pi1 && pi2 <= pj2)) continue;
        if((pi2 <= pj1 || pj2 <= pi1) && (linkMatrC[i1][j1 >> 3] & (1 << (j1 & 7))) == 0 && (linkMatrC[i2][j2 >> 3] & (1 << (j2 & 7))) == 0) {
            // i1-i2
            linkMatrC[i1][i2 >> 3] ^= 1 << (i2 & 7);
            linkMatrC[i2][i1 >> 3] ^= 1 << (i1 & 7);
            // j1-j2
        ...
        if((linkMatrC[i1][j2 >> 3] & (1 << (j2 & 7))) == 0 && (linkMatrC[i2][j1 >> 3] & (1 << (j1 & 7))) == 0) {
            // i1-i2
            linkMatrC[i1][i2] = 0;
            linkMatrC[i2][i1] = 0;
            // j1-j2
...
int exchange_link_deg_same(VVDistType& linkMatr, VNodeType& link, const VNodeType& p2pSize, LinkType count)
{
    const LinkType linkSize = link.size() / 2;
    if(linkSize < 2) return -1;
    while(count > 0) {
        ...
        NodeType pi1 = p2pSize[i1], pi2 = p2pSize[i2], pj1 = p2pSize[j1], pj2 = p2pSize[j2];
        if(pi1 >= pj2 || pj1 >= pi2 || i1 == j1 || i2 == j2) continue;
        if(pi1 != pj1 && pi2 != pj2 && ((pj1 < pi1 && pi2 < pj2) || (pi1 < pj1 && pj2 < pi2)) && linkMatr[i1][j2] == 0 && linkMatr[i2][j1] == 0) {
            // i1-i2
            linkMatr[i1][i2] = 0;
            linkMatr[i2][i1] = 0;
            // j1-j2
...
int exchange_link_deg_diff(VVDistType& linkMatr, VNodeType& link, const VNodeType& p2pSize, LinkType count)
{
    const LinkType linkSize = link.size() / 2;
    if(linkSize < 2) return -1;
    while(count > 0) {
        ...
        NodeType i1 = link[ik], i2 = link[ik | 1], j1 = link[jk], j2 = link[jk | 1];
        NodeType pi1 = p2pSize[i1], pi2 = p2pSize[i2], pj1 = p2pSize[j1], pj2 = p2pSize[j2];
        if(i1 == j2 || j1 == i2 || (pi1 <= pj1 && pj2 <= pi2) || (pj1 <= pi1 && pi2 <= pj2)) continue;
        if((pi2 <= pj1 || pj2 <= pi1) && linkMatr[i1][j1] == 0 && linkMatr[i2][j2] == 0) {
            // i1-i2
            linkMatr[i1][i2] = 0;
            linkMatr[i2][i1] = 0;
            // j1-j2
...
int exchange_linkC_num_same(VVChar& linkMatrC, VNodeType& link, LinkType count)
{
    ...
    while(count > 0) {
        ...
        if(((j1 < i1 && i2 < j2) || (i1 < j1 && j2 < i2)) && (linkMatrC[i1][j2 >> 3] & (1 << (j2 & 7))) == 0 && (linkMatrC[i2][j1 >> 3] & (1 << (j1 & 7))) == 0) {
            // i1-i2
            linkMatrC[i1][i2 >> 3] ^= 1 << (i2 & 7);
            linkMatrC[i2][i1 >> 3] ^= 1 << (i1 & 7);
            // j1-j2
...
int exchange_linkC_num_diff(VVChar& linkMatrC, VNodeType& link, LinkType count)
{
    ...
    while(count > 0) {
        ...
        jk <<= 1;
        NodeType i1 = link[ik], i2 = link[ik | 1], j1 = link[jk], j2 = link[jk | 1];
        if(i1 == j2 || j1 == i2 || (i1 <= j1 && j2 <= i2) || (j1 <= i1 && i2 <= j2)) continue;
        if((i1 > j2 || j1 > i2) && (linkMatrC[i1][j1 >> 3] & (1 << (j1 & 7))) == 0 && (linkMatrC[i2][j2 >> 3] & (1 << (j2 & 7))) == 0) {
            // i1-i2
            linkMatrC[i1][i2 >> 3] ^= 1 << (i2 & 7);
            linkMatrC[i2][i1 >> 3] ^= 1 << (i1 & 7);
            // j1-j2
...
int exchange_link_num_same(VVDistType& linkMatr, VNodeType& link, LinkType count)
{
    const LinkType linkSize = link.size();
    if(linkSize < 2) return -1;
    while(count > 0) {
        ...
        NodeType i1 = link[ik], i2 = link[ik | 1], j1 = link[jk], j2 = link[jk | 1];
        if(i1 >= j2 || j1 >= i2 || i1 == j1 || i2 == j2) continue;
        if(((j1 < i1 && i2 < j2) || (i1 < j1 && j2 < i2)) && linkMatr[i1][j2] == 0 && linkMatr[i2][j1] == 0) {
            // i1-i2
            linkMatr[i1][i2] = 0;
            linkMatr[i2][i1] = 0;
            // j1-j2
...
int exchange_link_num_diff(VVDistType& linkMatr, VNodeType& link, LinkType count)
{
    const LinkType linkSize = link.size();
    if(linkSize < 2) return -1;
    while(count > 0) {
        ...
        jk <<= 1;
        NodeType i1 = link[ik], i2 = link[ik | 1], j1 = link[jk], j2 = link[jk | 1];
        if((i1 <= j1 && j2 <= i2) || (j1 <= i1 && i2 <= j2) || i1 == j2 || i2 == j1) continue;
        if((i2 < j1 || j2 < i1) && linkMatr[i1][j1] == 0 && linkMatr[i2][j2] == 0) {
            // i1-i2
            linkMatr[i1][i2] = 0;
            linkMatr[i2][i1] = 0;
            // j1-j2
...
int count_sameAdiff(LinkType& sum, LinkType& sum1, LinkType& sum3,   LinkType& sum4, VLinkType& link, VNodeType& nodeDeg)
{
    const LinkType linkSize = link.size() / 2;
    sum = 0/*同配*/;
    sum1 = 0/*异配*/;
    ...
    for(LinkType l = 1; l < linkSize; l++) {
        ...
        for(LinkType ll = 0; ll < l; ll++) {
            ...
            NodeType di1 = nodeDeg[i1], di2 = nodeDeg[i2], dj1 = nodeDeg[j1], dj2 = nodeDeg[j2];
            if(di1 == di2 || dj1 == dj2) { //有某边两端点度相同 i-i j-k
                if(di1 == di2 && dj1 == dj2) { //两边的两端点度分别相同 i-i j-j
                    if(di1 == dj1)  sum4++; //度全相同 i==j
                    else sum++; //i!=j  同配
                    continue;
                }
                ...
                if(di1 > dj2 || dj1 > di2) sum++; //i-i,j-k : i<j<k || j<k<i
                else sum1++; //i-i,j-k: j<i<k
                continue;
            }
            //i-j k-l
            if(di1 >= dj2 || dj1 >= di2) sum++;
            else if((di1 <= dj1 && dj2 <= di2) || (dj1 <= di1 && di2 <= dj2)) sum1++;
            else sum++;
            continue;
        }
    }
...
int nodeDeg_update_nodeMap(VNodeType& nodeDeg, const VRNodeType& nodeMap)
{
    ...
    for(VRNodeTypeCItr i = nodeMap.begin(); i != nodeMap.end(); i++) t[i->end] = nodeDeg[i->start];
    swap(nodeDeg, t);
    return 0;
}
...
int read_weight_link(VVWeightType& vvweight, VVWeightType& vvweightIn, const LinkType linkSize, const char *name, const unsigned weight_m, const unsigned weight_n, const int dirFlag)
{
...
int read_link_weight_0(VNodeType& link, LinkType& linkSize, VVWeightType& vvweight, VVWeightType& vvweightIn, const char *name, const unsigned weight_m, const unsigned weight_n, const int dirFlag)
{
    if(weight_m < 2 || weight_m >= weight_n || name == NULL || name[0] == '\0' || !link.empty() || !vvweight.empty()){
        ERROR();
        return -1;
    }
    ifstream is(name);
