...
int common_read1(std::istream &is, std::vector<T>& v)
{
    ...
    for(typename std::vector<T>::iterator i = v.begin(); i != v.end() && is; i++) is >> *i;
    return 0;
}
...
{
    for (typename std::vector<std::vector<T> >::iterator i = v.begin(); i != v.end() && is; i++) {
        is >> *i;
    }
    return is;
}
...
int common_read2 (const char *name, std::vector<std::vector<T> > & v)
{
    ...
    for(typename std::vector<std::vector<T> >::iterator i = v.begin(); i != v.end() && is; i++) common_read1(is, *i);
    is.close();
    return 0;
}
...
void common_sort_p_val(Tp p, Tp p2, const T2 *const val)
{
    ...
    while(!st.empty()){
        ...
        if(val[*start] > pivot) pivot = val[*start];
        else if(val[*end] < pivot) pivot = val[*end];
        while(start < end){
        ...
        if(range.start + 1 <= start) st.push(Common_RangeP<Tp>(range.start, start));
