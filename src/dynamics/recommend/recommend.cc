...
// *******************************************************
int network::recommend::act_recommend_readP2p(
    VVNodeType& pu, VVNodeType& po, std::istream& is)
{
  if (!is) {
    ERROR();
    return -1;
...
int network::recommend::linkMatr_2_p2p2(VVNodeType& user_p_object,
    VVNodeType& object_p_user, const VVDistType& matr)
{
...
int network::recommend::linkMatr_2_userP2p(
    const VVDistType& matr, VVNodeType& userP2p)
{
  const NodeType NU = matr.size(), NO = matr[0].size();
  userP2p.clear();
  userP2p.resize(NU);
...
int network::recommend::linkMatr_2_objectP2p(
    const VVDistType& matr, VVNodeType& objectP2p)
{
  const NodeType NU = matr.size(), NO = matr[0].size();
  objectP2p.clear();
  objectP2p.resize(NO);
...
int network::recommend::act_recommend_mass_1(VDouble& o2,
    const VVNodeType& uP2p, const VVNodeType& oP2p, const NodeType i)
{
...
int network::recommend::act_recommend_mass(
...
int network::recommend::act_recommend_heat_1(VDouble& o2,
    const VVNodeType& uP2p, const VVNodeType& oP2p, const NodeType i)
{
  const NodeType NU = uP2p.size(), NO = oP2p.size();
  o2.assign(NO, 0);
...
int network::recommend::act_recommend_heat(
    VVDouble& rcm, const VVNodeType& uP2p, const VVNodeType& oP2p)
{
  const NodeType NU = uP2p.size();
  rcm.resize(NU);
...
// *******************************************************
int network::recommend::act_recommend_hybrid_matrix(VVDouble& rcm,
    const VVNodeType& uP2p, const VVNodeType& oP2p, const double lambda)
{
  const NodeType NU = uP2p.size(), NO = oP2p.size();
...
// *******************************************************
int network::recommend::act_recommend_pagerank(
    const VVNodeType& p2p, VDouble& v)
{
...
// *****************************************************
int network::recommend::act_recommend_commonNeighbour_object(
...
// *******************************************************
int network::recommend::recommend_MAE(
    const VDouble& v0, const VDouble& v, double& r)
{
  r = 0;
  if (v.size() != v0.size()) {
    ERROR();
...
int network::recommend::recommend_RMAE(
    const VDouble& v0, const VDouble& v, double& r)
{
  r = 0;
  if (v.size() != v0.size()) {
...
int network::recommend::recommend_NMAE(
    const VDouble& v0, const VDouble& v, double& r)
{
  r = 0;
  if (v.size() != v0.size()) {
...
int network::recommend::recommend_F(const VNodeType& v0, const VNodeType& v,
    double& recall, double& precision, double& F)
{
  NodeType s = 0;
...
// rs = sum(R[L...] / L.size())
int network::recommend::recommend_rankingScore(
