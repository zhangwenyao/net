...
// int network::fitness_complexity::count_newScale(const VNodeType& deg, const
// VVNodeType& newRemainRankV2, VVDouble& newScale)
//{
...
int network::fitness_complexity::save_rankScale(
  ...
  save1((s + "V2.txt").c_str(), rkV2, '\n');
  save1((s + "V2.index.txt").c_str(), rkV2Index, '\n');
  save1((s + "V2.scale.txt").c_str(), rkScale, '\n');
  return 0;
}
...
int network::fitness_complexity::save_rankScale2(const char* dir,
  ...
  save2((s + "V2.txt").c_str(), rkV2);
  save2((s + "V2.index.txt").c_str(), rkV2Index);
  save2((s + "V2.scale.txt").c_str(), rkScale);
  return 0;
}
...
int network::fitness_complexity::trade_name_OEC(
  ...
  VNodeType cIso, code;
  VString cIso3c, c3c;
  _ERR(read1_0(cIsoFile, cIso));
  _ERR(read1_0(cIso3cFile, cIso3c));
  c3c.resize(nc);
  code.assign(nc, 0);
...
int network::fitness_complexity::trade_name_NBERUN_wtf(
  ...
  _ERR(save1(countryFilename, cVal, '\n'));
  sort(pVal.begin(), pVal.end());
  _ERR(save1(productFilename, pVal, '\n'));
  return 0;
}
...
int network::fitness_complexity::sum_trade_OEC(const char* tradeFilename,
  ...
  for (double v; is >> year >> origin >> pd >> v; ++l) {
    ...
    if (year < year0) {
      ...
      if (year > year0) {
        INFORM(l, "\t", year0);
        _ERR(save2((name + to_string(year0) + ".txt").c_str(), expts));
        expts.assign(NC, VLinkType(NP, 0));
        year0 = year;
      }
      size_t c = cVal[atoi<size_t>(origin)], p = pVal[pd];
      _INF(expts[c][p] != 0);
      expts[c][p] = v;
    }
  }
...
int network::fitness_complexity::export_gdp_country_name(
  ...
  _ERR(save_VString(diffExportname, diffEpt));
  return 0;
}
...
int network::fitness_complexity::trade_gdp_country_code(
  ...
  _ERR(save1(diffGdpName, diffGdp, '\n'));
  sort(diffEpt.begin(), diffEpt.end());
  _ERR(save1(diffTradeName, diffEpt, '\n'));
  return 0;
}
...
int network::fitness_complexity::index_same_all(const char* namesFull,
                                                const char* sameAll) {
  {
    ...
    string s = namesFull;
    _ERR(read_VString((s + ".export.txt").c_str(), veptAll));
    _ERR(read_VString((s + ".same.common.txt").c_str(), vept));
    _ERR(read_VString((s + ".same.export.txt").c_str(), vept));
    VNodeType cIndex;
    ...
    s = sameAll;
    _ERR(save1((s + ".export.txt").c_str(), cIndex, '\n'));
  }

  {
    VString vgdp, vgdpAll;
    string s = namesFull;
    _ERR(read_VString((s + ".gdp.txt").c_str(), vgdpAll));
    _ERR(read_VString((s + ".same.common.txt").c_str(), vgdp));
    _ERR(read_VString((s + ".same.gdp.txt").c_str(), vgdp));
    VNodeType cIndex;
    ...
    s = sameAll;
    _ERR(save1((s + ".gdp.txt").c_str(), cIndex, '\n'));
  }
  return 0;
}
...
int network::fitness_complexity::index_same_all_OEC(
  ...
  VString c, cept, cgdp;
  _ERR(read_VString(sameName, c));
  _ERR(read_VString(sameTradeName, cept));
  _ERR(read_VString(sameGdpName, cgdp));
  for (VStringItr i = c.begin(); i != c.end(); i++) {
    cept.push_back(*i);
    cgdp.push_back(*i);
  }
  _ERR(save1(sameTradeAllName, cept, '\n'));
  _ERR(save1(sameGdpAllName, cgdp, '\n'));
...
int network::fitness_complexity::index_same_not0_OEC(
  ...
  _ERR(read1_0(tradeNot0IndexName, epN0));
  ...
  _ERR(save1(sameTradeNot0IndexName, e, '\n'));
  _ERR(save1(sameGdpNot0IndexName, g, '\n'));
  return 0;
}
...
int network::fitness_complexity::export_gdp_0_OEC(
  ...
  _ERR(read2_0(gFile, gdpAll));
  _ERR(read1_0(gCodeFile, gCode));
  _ERR(read1_0(egCodeFile, egCode));
  ...
  _ERR(save1(eg0CodeFile, eg0Code, '\n'));
  INFORM(cIndex.size());
  sort(pIndex.begin(), pIndex.end());
  _ERR(save1_p(eg0P4dFile, &p4d[0], &pIndex[0], pIndex.size(), '\n'));
  INFORM(pIndex.size());
...
int network::fitness_complexity::population_OEC(
  ...
  _ERR(read1_0(pIsoCodeFile, pIsoCode));
  ...
  for (NodeType ci = 0; ci < NC; ++ci) {
    ...
    // p += populations[ci][yi];
    // if (find(c0IsoCode, pIsoCode[ci]) && p >= filter * 3)
    // pIndex.push_back(ci);
    ...
    // break;
    //}
    // if (flag && find(c0IsoCode, pIsoCode[ci]))
    // cIndex.push_back(ci);
  }
  _ERR(save1_p(pfFile, &pIsoCode[0], &cIndex[0], cIndex.size(), '\n'));
  INFORM(cIndex.size());
  return 0;
}
...
int network::fitness_complexity::index_export_gdp(
  ...
  _ERR(read1_0(exportIndexFile0, e0));
  _ERR(read1_0(exportIndexFile, e));
  if (g0.size() != e0.size()) {
    ERROR();
    return -1;
  }
...
int network::fitness_complexity::data(const char* gdpFile,
  ...
  _ERR(read2_0(gdpFile, gdp));
  VNodeType gIndex;
  _ERR(read1_0(gdpIndexFile, gIndex));
  VNodeType cIndex, pIndex;
  _ERR(read1_0(countryIndexFile, cIndex));
  _ERR(read1_0(productIndexFile, pIndex));
  ...
  for (NodeType i = 0; i < NYEAR; ++i) {
    ...
    VVString epts;
    string s0 = exportDIR;
    _ERR(read2_0((s0 + y + ".export.txt").c_str(), epts));
    VVString epts2(cIndex.size());
    for (NodeType ci = 0; ci < cIndex.size(); ++ci)
      for (NodeType pi = 0; pi < pIndex.size(); ++pi)
        epts2[ci].push_back(epts[cIndex[ci]][pIndex[pi]]);
    _ERR(save2((s + y + ".export.txt").c_str(), epts2));
  }
  return 0;
}
...
int network::fitness_complexity::code2name(const char* nameFile,
  ...
  _ERR(read1_0(codeFile, code));
  _ERR(read1_0(code0File, code0));
  ...
  _ERR(save_VString(name0File, name0));
  return 0;
}
