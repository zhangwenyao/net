...
//	g++ cluster.cpp -o cluster.exe -Wall -O2 -L/usr/X11R6/lib -lm -lpthread
//-lX11 ; ./cluster.exe -DD_T=2.e-0 -DMAX_T=2.e2
#include <cmath>
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
/*******************	类型声明定义 *********************/
//	粒子密度：d	格点网络大小：L*L  粒子数目：N=L*L*d
// leader比例：yita
#ifndef L
#define L 193
#endif
...
inline int R_DIR(const int dir) { return 5 - dir; }
const char EMPTY_CHAR = '.', UP_CHAR = '^', DOWN_CHAR = 'v', LEFT_CHAR = '<',
           RIGHT_CHAR = '>';
const char dirOut[DIR_SIZE]
    = { EMPTY_CHAR, UP_CHAR, LEFT_CHAR, RIGHT_CHAR, DOWN_CHAR };
...
class Par {
  ...
  //	按变化概率处于状态sta[staId][staId2],sta[STA_SIZE-1][staMvId]
  //转移概率pt:{EMPTY,UP,LEFT,RIGHT,DOWN}
  Pos pos;
  int dir;
  bool moveFlag;
  int dud, dlr;
...
extern Par
    par[N + 1]; //	给粒子从1开始编号至N(0号不用)，数组par[N+1]保存粒子参数
...
void parRand(int* const pNum, int* const pDir,
    double* const tao); //	返回随机的改变状态的粒子编号及方向
...
int N0, dirNum[DIR_SIZE] = { 0 }; //	N0：leader数目
                                  //dirNum粒子在各方向的数目
...
double pt[STA_SIZE],
    ptDir[STA_DIR_SIZE][STA_DIR_SIZE]
         [DIR_SIZE]; //	按各粒子总概率pt可分为staSize种粒子，相应概率为pt
                     // prDir[ud][lr][toDir]
...
FILE* fpLog;
double t = 0.;
long long unsigned tCount = 0, moveCount = 0, tCountD,
                   moveCountD; //	移动的次数
...
unsigned char *color[DIR_SIZE] = { white, red, green, blue, yellow },
              *colorLeader[DIR_SIZE] = { white, red, green, blue, yellow };
#endif
...
void parPrint(void)
{
  ...
  double* pp;
  printf("par	size:	%d\n", N);
  printf("No.	pos:x,y	  dir	mvFlg	staMvId	dud	dlr	staId	"
         "staId2	pt %c	pt %c	pt %c	pt %c\n",
      dirOut[1], dirOut[2], dirOut[3], dirOut[4]);
  for (i = 1; i <= N; ++i, ++p) {
    pp = ptDir[p->dud + DIR_SIZE - 1][p->dlr + DIR_SIZE - 1];
    printf("%d	%3d,%3d	  %c	%d	%d	%d	%d	%d	"
           "%d	%6.2lf	%6.2lf	%6.2lf	%6.2lf\n",
        i, p->pos.x, p->pos.y, dirOut[p->dir], p->moveFlag, p->staMvId, p->dud,
        p->dlr, p->staId, p->staId2, pp[1], pp[2], pp[3], pp[4]);
  }
  return;
}
...
void saveData(char path[], char name[])
{
  ...
  if (tCountD > 0)
    ...
    mMove = 0.;
  m = sqrt((dirNum[UP] - dirNum[DOWN]) * (dirNum[UP] - dirNum[DOWN])
          + (dirNum[LEFT] - dirNum[RIGHT]) * (dirNum[LEFT] - dirNum[RIGHT]))
      / N;
  printf("%g	%g	%g	%llu	%llu	%d	%d	%d	%d\n",
      t, m, mMove, tCount, moveCount, dirNum[UP], dirNum[LEFT], dirNum[RIGHT],
      dirNum[DOWN]);
  if (fpLog != NULL)
    fprintf(fpLog,
        "%g	%g	%g	%llu	%llu	%d	%d	%d	%d\n",
        t, m, mMove, tCount, moveCount, dirNum[UP], dirNum[LEFT],
        dirNum[RIGHT], dirNum[DOWN]);
  ...
  if (fps[UP] == NULL || fps[LEFT] == NULL || fps[RIGHT] == NULL
      || fps[DOWN] == NULL) {
...
void MainInit(int argc, char* argv[])
{
  ...
  for (i = 1; i < argc; ++i) {
    if (0 == strcmp(argv[i], "--help") || 0 == strcmp(argv[i], "-help")
        || 0 == strcmp(argv[i], "--h") || 0 == strcmp(argv[i], "-h")) {
      printf("%s\n", argv[0]);
      printf("%s\n", "options usage:	[-option arg]	or	[--option "
                     "arg]");
      ...
      MainExit();
      exit(0);
    } else if (0 == strcmp(argv[i], "--yita")
        || 0 == strcmp(argv[i], "-yita")) {
      sscanf(argv[++i], "%lf", &yita);
      if (yita < 0.)
        yita = 0.;
      else if (yita > 1.)
        yita = 1.;
    } else if (0 == strcmp(argv[i], "--MAX_T")
        || 0 == strcmp(argv[i], "-MAX_T")) {
  ...
  char cmdName[128] = "";
  sprintf(dirName, "./data/L_%d_d_%g_N_%d_N0_%d_g_%g_seed_%u", L, d, N, N0, g,
      seed);
  ...
  printf("t	m	mMove	tCount	moveCount	UP	LEFT	"
         "RIGHT	DOWN\n");
  if (fpLog != NULL)
    fprintf(fpLog, "L=%d	LL=%d	d=%g	N=%d	N0=%d	g=%g	"
                   "seed=%u\ndirname : %s\n%s\n",
        L, LL, d, N, N0, g, seed, dirName, "t	m	mMove	tCount	"
                                           "moveCount	UP	LEFT	"
                                           "RIGHT	DOWN\n");
...
void staUpdate(const int num)
{
  ...
  if (pPar->staId != STA_NULL) {
    num2 = sta[pPar->staId][pPar->staId2]
        = sta[pPar->staId][sta[pPar->staId][0]--];
