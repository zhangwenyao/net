...
template <typename T>
int common::read1(std::istream& is, std::vector<T>& v, const bool info) {
  _ERR(!is);
  for (auto& i : v) is >> i;
  return 0;
}
...
template <typename T>
int common::read1(const char* name, std::vector<T>& v, const bool info) {
  std::ifstream is(name);
  if (!is) {
    if (info) ERROR("open file ", name);
    return -1;
  }
