    ...
    const VDouble& degArrProb, LinkType& linkSize)
{
  ...
  NodeType k1, k2;
  double sum = 0;
  ::common_total(&degArrProb[0], degSize, sum);
  while (1) {
    while (1) {
      k1 = ::random_new_prob_sum(&degArrProb[0], degSize, sum);
      if (degArrSize[k1] > 0 && (k1 != 0 || degArrSize[0] > 1)
          && (k1 != degSize - 1 || degArrSize[degSize - 1] > 1))
        break;
    }
    while (1) {
      k2 = ::random_new_prob_sum(&degArrProb[0], degSize, sum);
      if (k2 <= k1)
        ++k2;
      if (k2 != k1)
        break;
    ...
    const VDouble& degArrProbSum, LinkType& linkSize)
{
  ...
  while (1) {
    while (1) {
      k1 = ::random_new_probSum(&degArrProbSum[0], degSize);
      if (degArrSize[k1] > 0 && (k1 != 0 || degArrSize[0] > 1)
          && (k1 != degSize - 1 || degArrSize[degSize - 1] > 1))
        break;
    }
    while (1) {
      k2 = ::random_new_probSum(&degArrProbSum[0], degSize);
      if (k2 != k1)
        break;
    }
    if (degArrVal[k1] % 2 != degArrVal[k2] % 2)
