...
for  i in `seq 0 9`; do
  sleep 1
  echo "nohup ./bin/net.exe --config config/config_EmailEnron_beta${beta}_gamma1_$i.json > logs/EmailEnron_beta${beta}_gamma1_$i.log 2>&1 &"
  nohup ./bin/net.exe --config config/config_EmailEnron_beta${beta}_gamma1_$i.json > logs/EmailEnron_beta${beta}_gamma1_$i.log 2>&1 &
done
