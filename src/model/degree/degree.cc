...
// *******************************************************************
// 生成度分布概率
int network::degree::poisson_cal_degArrProb(const double p,
    VNodeType& degArrVal, VDouble& degArrProb, const NodeType kMin,
...
// 生成度分布概率
int network::degree::power_cal_degArrProb(const double r,
    VNodeType& degArrVal, VDouble& degArrProb, const NodeType kMin,
    const NodeType kMax)
{
  const NodeType kSize = kMax - kMin + 1;
...
// 生成度分布累计序列
int network::degree::power_cal_deg_arr(VNodeType& degArrSize,
...
// 生成度分布累计序列
int network::degree::power_cal_deg_arr_func(const NodeType nodeSize,
...
int network::degree::read_prob_sum(
...
// 生成度分布累计序列
int network::degree::power_cal_deg_arr_prob_sum_arr(const NodeType nodeSize,
    const NodeType kMin, const NodeType kMax,
    ...
    VNodeType& degArrVal, VNodeType& degArrSize)
{
  ...
  if (size)
    _ERR(power_cal_deg_arr_prob_sum_arr2(size, n1, prob_func, degProbSumVal,
        degProbSumArr, degArrVal, degArrSize));
  _ERR(degArrVal.size() <= 0 || degArrVal.front() != kMin
      || degArrVal.back() != kMax);
  return 0;
}

int network::degree::power_cal_deg_arr_prob_sum_arr2(const NodeType nodeSize,
    ...
    VNodeType& degArrVal, VNodeType& degArrSize)
{
  ...
  // if (size)
  //_ERR(network::degree::power_cal_deg_arr_prob_sum_arr3(size, n1, prob_func,
  // degProbSumVal, degProbSumArr, degArrVal, degArrSize));
  _ERR(size > 0 || degArrVal.size() <= 0 || degArrVal.back() != kMax);
  return 0;
}

int network::degree::power_cal_deg_arr_prob_sum_arr3(const NodeType nodeSize,
    const NodeType kMinN, std::function<Double(const NodeType)> prob_func,
    const VNodeType& degProbSumVal, const VDouble& degProbSumArr,
    VNodeType& degArrVal, VNodeType& degArrSize)
{
...
int network::degree::fix_degArr_linkSize_lkk3_fast(
...
// ********************************************************
int network::degree::power_nature_cutoff(NodeType& kMax,
    const NodeType nodeSize, const NodeType kMin, const double gamma)
{
