...
// **************************************************************
int network::grid::Grid_new(const NodeType NX, const NodeType NY,
    NodeType& kMin, NodeType& kMax, NodeType& nodeSize, VVNodeType& p2p)
{
  if (NX <= 0 || NY <= 0)
    return -1;
  kMin = 2;
...
int network::grid::Grid_check(const NodeType NX, const NodeType NY,
...
int network::grid::Grid_get_path(VNodeType& path, const NodeType ni,
    ...
    const NodeType NX, const NodeType NY)
{
  ...
  for (NodeType i = 0; i < dy; i++)
    *p++ = yDir;
  if (dx > 0 && dy > 0)
    new_randArrAll(&path[1], d); //每一步的可能方向均随机取
...
int network::grid::Grid_get_shortPath(VNodeType& path, const NodeType ni,
    const NodeType nj, const NodeType nodeSize, const VVNodeType& p2p,
    const NodeType NX, const NodeType NY)
{
  if (ni >= nodeSize || nj >= nodeSize) {
