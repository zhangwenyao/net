...
int cal_pearson(double& pearson, const VVNodeType& p2p,
...
int cal_pearson_dir_weight(double& pearson, double& rho, const VVNodeType& p2p,
                           const VVWeightType& vvweight,
...
int cal_pearson_link(double& pearson, const VNodeType& link,
                     const VNodeType& nodeDeg) {
  ...
  if (n * sxx - sx * sx == 0)
    ...
    pearson = (n * 2. * sxy - sx * sx) / (n * sxx - sx * sx);
  return 0;
}
