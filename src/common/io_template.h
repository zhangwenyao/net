...
int common::read0map(std::istream &is, std::map<T1, T2> &m)
{
  _ERR(!is);
  T1 t1;
  T2 t2;
...
template <typename T1, typename T2>
int common::read0map(const char *name, std::map<T1, T2> &m)
{
  std::ifstream is(name);
  if (!is)
  {
  ...
  int status = common::read0map<T1, T2>(is, m);
  is.close();
  return status;
}

template <typename T1, typename T2>
int common::read0set(std::istream &is, std::vector<std::set<T2>> &s, const std::map<T1, T2> &u, const std::map<T1, T2> &v)
{
  _ERR(!is);
  T1 t1, t2;
  while (is >> t1 >> t2)
...
template <typename T1, typename T2>
int common::read0set(const char *name, std::vector<std::set<T2>> &s, const std::map<T1, T2> &u, const std::map<T1, T2> &v)
{
  std::ifstream is(name);
  if (!is)
  {
  ...
  int status = common::read0set<T1, T2>(is, s, u, v);
  is.close();
  return status;
}
