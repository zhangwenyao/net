...
int net_random_init(Network& net)
{
    ...
    if(net.kMin > net.kMax || net.kMax >= net.nodeSize || (net.kMin == net.kMax && net.nodeSize % 2 == 1 && net.kMin % 2 == 1) || net.params_random.p < 0. || net.params_random.p > 1.) return net.status = -1;
    return 0;
}
...
int net_ER(Network& net)
{
    ...
    int p = net.params_random.p * RAND2_MAX;
    net.p2p.clear();
...
{
    for(string s; is >> s;){
        if (s == "--params_random.p") {
            is >> net.params_random.p;
            cout << s << '\t'   << net.params_random.p << endl;
            continue;
        }
    }
    return 0;
...
{
    if(!os) return -1;
    os << "--params_random.p\t" << net.params_random.p
        << '\n';
    return 0;
}
...
int net_random_node_prob(Network& net) // 所有点按概率p连边
{
    ...
    // 连边
    net.linkSize = 0;
    addLink_linkMatr_proNode(net.linkMatr, net.linkSize, net.params_random.p, net.dirFlag); // 连边
    net.linkRemain = 0;
...
int net_random_ranNode(Network& net)   // 每次直接随机抽取两个点连边
{
    ...
    // 连边
    net.linkRemain = net.linkSize = net.params_random.p * (net.nodeSize - 1) * net.nodeSize / 2;
