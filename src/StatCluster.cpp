...
int cal_cluster_directed_weight(double &cluster_c, VDouble &cluster, VVWeightType& weightMatr, const VVDistType& linkMatr)
{
    ...
    for(NodeType nodeI = 0; nodeI < nodeSize; nodeI++){
        ...
        for(VNodeTypeCItr j = p2p[nodeI].begin(), end = p2p[nodeI].end(); j != end; j++){
    ...
    for(NodeType i = 0; i < nodeSize; i++)
        if(cluster[i] != 0) cluster_c += cluster[i] /= K[i] * (K[i] - 1.) - 2 * A2[i];
    cluster_c /= nodeSize;
...
int cal_cluster_directed_unweight(double &cluster_c, VDouble &cluster, const VVDistType& linkMatr)
{
    ...
    for(VVNodeTypeCItr i = p2p.begin(); i != p2p.end(); nodeI++, i++){
        for(VNodeTypeCItr j = i->begin(); j != i->end(); j++){
            const NodeType nodeJ = *j;
            for(VNodeTypeCItr jj = p2p[nodeJ].begin(); jj != p2p[nodeJ].end(); jj++){
                if(S[*jj][nodeI] != 0){
                    const double t = S[nodeJ][nodeI] * S[*jj][nodeJ] * S[*jj][nodeI];
                    cluster[nodeI] += t;
                    cluster[nodeJ] += t;
