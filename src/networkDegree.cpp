...
{
    // 生成度分布概率
    poisson_cal_degArr(net.Poi_p, net.degArrVal, net.degArrProb, net.kMin, net.kMax, net.nodeSize);
...
{
    if(net.kMin > net.kMax || net.kMax >= net.nodeSize || (net.kMin == net.kMax && net.nodeSize % 2 == 1 && net.kMin % 2 == 1) || net.Pow_gamma < 0) return -1;
    return 0;
}
...
{
    // 生成度分布概率
    power_cal_degArr(net.Pow_gamma, net.degArrVal, net.degArrProb, net.kMin, net.kMax);
    net.nodeDeg.resize(net.nodeSize);
    random_new_randKArr(&net.nodeDeg[0], (size_t)net.nodeSize, &net.degArrProb[0], &net.degArrVal[0], net.degArrVal.size()); // 调用通用随机函数
    // 修正度序列使总数为偶数
    if(nodeDeg_2_linkSize(net.linkSize, net.nodeDeg) != 0){
