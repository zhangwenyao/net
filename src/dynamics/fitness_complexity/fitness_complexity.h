...
template <typename T>
int Mcp_2_C_P(const std::vector<std::vector<T>>& mcp, VVNodeType& pc,
              VVNodeType& pp);
template <typename T>
int Mcp_2_FC(VDouble& Fc, VDouble& Cp, const std::vector<std::vector<T>>& Mcp);
template <typename T>
...
int count_product_deg(const VVNodeType& mcp, VNodeType& deg,
...
int count_newRank(const VVDouble& rcm, const size_t NC, const size_t NP,
                  const VVNodeType& mcpNew, VVNodeType& newRank,
                  const char* name = NULL);
...
int cal_val_2_rankScale(const VDouble& val, VNodeType& rk, VNodeType& rkIndex,
                        VNodeType& rkV2, VNodeType& rkV2Index, VDouble& rkScale,
                        const size_t N);
int cal_val_2_rankScale_p(const VDouble& val, const NodeType* p, VNodeType& rk,
                          VNodeType& rkIndex, VNodeType& rkV2,
                          VNodeType& rkV2Index, VDouble& rkScale,
                          const size_t N);
...
int save_rankScale2_val_p(const char* dir, const VVDouble& val,
                          const VVNodeType p);
...
// 合并进出口数据
int read_country_names(const char* countryFilename, NodeType& NC,
                       VNodeType& cVal);
int read_product_names(const char* productFilename, NodeType& NP,
                       VNodeType& pVal);
...
int sum_trade_NBER_wtf(const char* tradeFilename, const char* countryFilename,
                       const char* productFilename, const char* saveName);
int sum_trade_NBER_wtf2(const char* tradeFilename, const char* countryFilename,
                        const char* productFilename, const char* saveName);
...
int index_export_0_NBER_wtf(const char* epDIR, const char* cIndexFile,
                            const char* pIndexFile,
                            const char* pIndexFileAll = NULL,
                            const NodeType YEAR1 = 1984,
                            const NodeType YEAR2 = 2001,
...
int data(const char* gdpFile, const char* gdpIndexFile, const char* exportFile,
...
int trade_gdp_country_code_OEC(const char* eCodeFile, const char* gCodeFile,
                               const char* codeFile);
int index_same_all_OEC(const char* tradeName, const char* gdpName,
                       ...
                       const char* sameGdpAllName,
                       const char* sameTradeAllIndexName,
                       const char* sameGdpAllIndexName);
int index_same_not0_OEC(const char* sameTradeAllIndexName,
                        const char* sameGdpAllIndexName,
                        const char* tradeNot0IndexName,
                        const char* sameTradeNot0IndexName,
                        const char* sameGdpNot0IndexName);
int index_export_0_OEC(const char* epDIR, const char* cIndexFile,
                       const char* pIndexFile,
                       const char* countryFilename = NULL,
                       const char* productFilename = NULL,
                       const NodeType YEAR1 = 2001,
                       const NodeType YEAR2 = 2015);
int export_gdp_0_OEC(const char* epDIR, const NodeType YEAR1,
                     const NodeType YEAR2, const char* p4dFile,
                     const char* cCodeFile, const char* c3cCodeFile,
                     const char* gFile, const NodeType YEAR0,
                     const char* gCodeFile, const char* egCodeFile,
                     const char* egCCodeFile, const char* egP4dFile,
                     const LinkType filter = 0);
...
int trade_OEC(const char* epDir, const NodeType YEAR_FILTER1,
              ...
              const char* c0IsoCodeFile, const LinkType filter,
              const char* cfFile);
int code2name(const char* nameFile, const char* codeFile, const char* code0File,
              const char* name0File);
int data_export_OEC(const char* cp0DIR, const char* c0CodeFile,
                    const char* p4d0File, const char* cCodeFile,
                    const char* p4dFile, const NodeType YEAR1,
                    const NodeType YEAR2, const char* cpDIR,
                    const char* cpName);
...
int filter_data2_code2(const char* d0File, const char* code0File,
                       const char* codeFile, const char* dFile_dir,
                       const char* dFile_dir2, const size_t y1, const size_t y2,
                       const size_t y0);
}  // namespace fitness_complexity
}  // namespace network
// *******************************************************
