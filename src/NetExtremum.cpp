...
int cal_Min_lkk(VVLinkType& lkk, const VNodeType& degArrVal,
                const VNodeType& degArrSize, const VLinkType& linkSum,
                const int fix) {
  ...
  for (NodeType left = mid - (linkMid == 0 && mid > 0),
    ...
    // 从中间mid往两端修正
    if (cal_Min_lkk_fix(lkk, degArrVal, degArrSize, degSize, vlink, left, right,
                        mid, linkMid) == -1) {
      ERROR();
      return -1;
    }
...
int cal_Min_lkk_fix(VVLinkType& lkk, const VNodeType& degArrVal,
                    const VNodeType& degArrSize, const NodeType degSize,
                    VLinkType& vlink, const NodeType& left,
                    const NodeType& right, const NodeType& mid,
                    LinkType& linkMid) {
  ...
  for (NodeType l = left; linkRight && l > 0;) {
    l--;
    LinkType links = (LinkType)(degArrSize[l] < degArrVal[right]
                                    ? degArrSize[l]
                                    : degArrVal[right]) *
                     degArrSize[right],
...
int Min_new_lkk_p2p(VVNodeType& p2p, VVLinkType& lkk, const VNodeType& nodeDeg,
                    const VNodeType& degArrVal, const VNodeType& degArrSize,
                    const VNodeType& degArrSum, VLinkType& linkSum,
                    const int fix) {
  if (degArrSize.empty()) {
    ERROR();
    return -1;
...
int Min_new_lkk(VVLinkType& lkk, const VNodeType& degArrVal,
                const VNodeType& degArrSize, VLinkType& linkSum,
                const int fix) {
  if (degArrSize.empty()) {
    ERROR();
    return -1;
